/**
 * Accessory / Parts Requisition Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  ACCESSORY_REQUISITION_FORM_GROUPS,
  ACCESSORY_REQUISITION_FORM_SECTIONS,
...
export const ACCESSORY_REQUISITION_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const ACCESSORY_REQUISITION_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampAccessoryRequisitionFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampAccessoryRequisitionFormColSpan(value, columns = 2) {
  const max = clampAccessoryRequisitionFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampAccessoryRequisitionFormColumns(columns);
  // Accessory / Parts Requisition defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampAccessoryRequisitionFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const ACCESSORY_REQUISITION_FORM_LEVEL_1 = ACCESSORY_REQUISITION_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampAccessoryRequisitionFormColSpan(
...
/**
 * Seed hierarchy: Main holds Category, Number of items, Item, Quantity, and Justification.
 */
export function buildInitialAccessoryRequisitionFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return ACCESSORY_REQUISITION_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampAccessoryRequisitionFormColumns(section.columns ?? 2);
    const groups = ACCESSORY_REQUISITION_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = ACCESSORY_REQUISITION_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampAccessoryRequisitionFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = ACCESSORY_REQUISITION_FORM_FIELD_CATALOG.filter(
...
export function cloneAccessoryRequisitionFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneAccessoryRequisitionFormTree(node.children || []),
  }));
}

export function isAccessoryRequisitionFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isAccessoryRequisitionFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenAccessoryRequisitionFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenAccessoryRequisitionFormTree(node.children, acc);
  });
  return acc;
}

export function findAccessoryRequisitionFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findAccessoryRequisitionFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findAccessoryRequisitionFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findAccessoryRequisitionFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getAccessoryRequisitionFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findAccessoryRequisitionFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findAccessoryRequisitionFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getAccessoryRequisitionFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findAccessoryRequisitionFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findAccessoryRequisitionFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getAccessoryRequisitionFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getAccessoryRequisitionFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveAccessoryRequisitionFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isAccessoryRequisitionFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveAccessoryRequisitionFormLeaves(child));
}

export function collectAllActiveAccessoryRequisitionFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveAccessoryRequisitionFormLeaves(node));
}
...
export function filterAccessoryRequisitionFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getAccessoryRequisitionFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenAccessoryRequisitionFormTree(nodes)
    .filter((node) => {
      if (isAccessoryRequisitionFormValueField(node)) return false;
...
export function addAccessoryRequisitionFormNode(nodes = [], payload) {
  const nextTree = cloneAccessoryRequisitionFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findAccessoryRequisitionFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isAccessoryRequisitionFormValueField(parent)) {
  ...
  if (findAccessoryRequisitionFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getAccessoryRequisitionFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampAccessoryRequisitionFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampAccessoryRequisitionFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampAccessoryRequisitionFormColSpan(
...
export function updateAccessoryRequisitionFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneAccessoryRequisitionFormTree(nodes);
  const existing = findAccessoryRequisitionFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampAccessoryRequisitionFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampAccessoryRequisitionFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findAccessoryRequisitionFormParentNode(nextTree, nodeId);
    const sectionColumns = clampAccessoryRequisitionFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampAccessoryRequisitionFormColSpan(
  ...
  const isLeaf = isAccessoryRequisitionFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getAccessoryRequisitionFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampAccessoryRequisitionFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampAccessoryRequisitionFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampAccessoryRequisitionFormColSpan(
...
export function toggleAccessoryRequisitionFormActive(nodes = [], nodeId) {
  const nextTree = cloneAccessoryRequisitionFormTree(nodes);
  const existing = findAccessoryRequisitionFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleAccessoryRequisitionFormHidden(nodes = [], nodeId) {
  const nextTree = cloneAccessoryRequisitionFormTree(nodes);
  const existing = findAccessoryRequisitionFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteAccessoryRequisitionFormNode(nodes = [], nodeId) {
  const existing = findAccessoryRequisitionFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneAccessoryRequisitionFormTree(nodes));
}

export function reorderAccessoryRequisitionFormNode(nodes = [], nodeId, direction) {
  const existing = findAccessoryRequisitionFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneAccessoryRequisitionFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderAccessoryRequisitionFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findAccessoryRequisitionFormNodeById(nodes, sourceId);
...
export function getAccessoryRequisitionFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Accessory / Parts Requisition. */
export function leafToAccessoryRequisitionField(leaf, sectionId) {
