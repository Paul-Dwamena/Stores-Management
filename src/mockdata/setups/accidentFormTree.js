/**
 * Accident form hierarchy — fixed Level 1 wizard steps with locked
 * Level 2 subfolders (collapsible sections) and leaf value fields.
 */

import { moveTreeSiblingRelative, reorderTreeSibling } from "../../utils/treeReorder";
import {
  DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION,
  ACCIDENT_FORM_FIELD_CATALOG,
  ACCIDENT_FORM_GROUPS,
  ACCIDENT_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./accidentFormFields";
import {
...
export const ACCIDENT_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const ACCIDENT_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampAccidentFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampAccidentFormColSpan(value, columns = 2) {
  const max = clampAccidentFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampAccidentFormColumns(columns);
  // Link Fuel Card defaults new fields to full row width (vehicle/driver stay half in catalog).
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampAccidentFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const ACCIDENT_FORM_LEVEL_1 = ACCIDENT_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
/**
 * Seed hierarchy: L1 wizard steps → L2 collapsible groups → L3 leaf fields.
 */
export function buildInitialAccidentFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return ACCIDENT_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampAccidentFormColumns(section.columns ?? 1);
    const groups = ACCIDENT_FORM_GROUPS.filter(
    ...
    return {
      ...
      children: groups.map((group, groupIndex) => {
        const groupColumns = clampAccidentFormColumns(group.columns ?? 2);
        const groupNodeId = `${section.id}/${group.id}`;
        const fields = ACCIDENT_FORM_FIELD_CATALOG.filter(
        ...
        return {
          ...
          children: fields.map((field, fieldIndex) => {
            ...
            return {
              ...
              columns: null,
              colSpan: clampAccidentFormColSpan(
                field.colSpan ?? defaultColSpanForFieldType(fieldType, groupColumns),
                groupColumns,
              ),
...
export function cloneAccidentFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneAccidentFormTree(node.children || []),
  }));
}

export function isAccidentFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isAccidentFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenAccidentFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenAccidentFormTree(node.children, acc);
  });
  return acc;
}

export function findAccidentFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findAccidentFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findAccidentFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findAccidentFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getAccidentFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findAccidentFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findAccidentFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getAccidentFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findAccidentFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findAccidentFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getAccidentFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getAccidentFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveAccidentFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isAccidentFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveAccidentFormLeaves(child));
}

export function collectAllActiveAccidentFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveAccidentFormLeaves(node));
}
...
export function filterAccidentFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getAccidentFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenAccidentFormTree(nodes)
    .filter((node) => {
      if (isAccidentFormValueField(node)) return false;
...
export function addAccidentFormNode(nodes = [], payload) {
  const nextTree = cloneAccidentFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findAccidentFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isAccidentFormValueField(parent)) {
  ...
  if (findAccidentFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getAccidentFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampAccidentFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampAccidentFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampAccidentFormColSpan(
...
export function updateAccidentFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneAccidentFormTree(nodes);
  const existing = findAccidentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampAccidentFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampAccidentFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findAccidentFormParentNode(nextTree, nodeId);
    const sectionColumns = clampAccidentFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampAccidentFormColSpan(
  ...
  const isLeaf = isAccidentFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getAccidentFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampAccidentFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampAccidentFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampAccidentFormColSpan(
...
export function toggleAccidentFormActive(nodes = [], nodeId) {
  const nextTree = cloneAccidentFormTree(nodes);
  const existing = findAccidentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleAccidentFormHidden(nodes = [], nodeId) {
  const nextTree = cloneAccidentFormTree(nodes);
  const existing = findAccidentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteAccidentFormNode(nodes = [], nodeId) {
  const existing = findAccidentFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneAccidentFormTree(nodes));
}

export function reorderAccidentFormNode(nodes = [], nodeId, direction) {
  const { nodes: next, moved } = reorderTreeSibling(
    cloneAccidentFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderAccidentFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
...
export function getAccidentFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the legacy field shape used by AccidentFormWizard. */
export function leafToAccidentField(leaf, sectionId) {
