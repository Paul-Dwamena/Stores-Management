/**
 * Add Fuel Vendor Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  ADD_FUEL_VENDOR_FORM_FIELD_CATALOG,
  ADD_FUEL_VENDOR_FORM_GROUPS,
  ADD_FUEL_VENDOR_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./addFuelVendorFormFields";
import {
...
export const ADD_FUEL_VENDOR_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const ADD_FUEL_VENDOR_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampAddFuelVendorFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampAddFuelVendorFormColSpan(value, columns = 2) {
  const max = clampAddFuelVendorFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampAddFuelVendorFormColumns(columns);
  // Add Fuel Vendor defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampAddFuelVendorFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const ADD_FUEL_VENDOR_FORM_LEVEL_1 = ADD_FUEL_VENDOR_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampAddFuelVendorFormColSpan(
 ...
 */
export function buildInitialAddFuelVendorFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return ADD_FUEL_VENDOR_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampAddFuelVendorFormColumns(section.columns ?? 2);
    const groups = ADD_FUEL_VENDOR_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = ADD_FUEL_VENDOR_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampAddFuelVendorFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = ADD_FUEL_VENDOR_FORM_FIELD_CATALOG.filter(
...
export function cloneAddFuelVendorFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneAddFuelVendorFormTree(node.children || []),
  }));
}

export function isAddFuelVendorFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isAddFuelVendorFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenAddFuelVendorFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenAddFuelVendorFormTree(node.children, acc);
  });
  return acc;
}

export function findAddFuelVendorFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findAddFuelVendorFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findAddFuelVendorFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findAddFuelVendorFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getAddFuelVendorFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findAddFuelVendorFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findAddFuelVendorFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getAddFuelVendorFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findAddFuelVendorFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findAddFuelVendorFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getAddFuelVendorFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getAddFuelVendorFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveAddFuelVendorFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isAddFuelVendorFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveAddFuelVendorFormLeaves(child));
}

export function collectAllActiveAddFuelVendorFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveAddFuelVendorFormLeaves(node));
}
...
export function filterAddFuelVendorFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getAddFuelVendorFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenAddFuelVendorFormTree(nodes)
    .filter((node) => {
      if (isAddFuelVendorFormValueField(node)) return false;
...
export function addAddFuelVendorFormNode(nodes = [], payload) {
  const nextTree = cloneAddFuelVendorFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findAddFuelVendorFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isAddFuelVendorFormValueField(parent)) {
  ...
  if (findAddFuelVendorFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getAddFuelVendorFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampAddFuelVendorFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampAddFuelVendorFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampAddFuelVendorFormColSpan(
...
export function updateAddFuelVendorFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneAddFuelVendorFormTree(nodes);
  const existing = findAddFuelVendorFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampAddFuelVendorFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampAddFuelVendorFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findAddFuelVendorFormParentNode(nextTree, nodeId);
    const sectionColumns = clampAddFuelVendorFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampAddFuelVendorFormColSpan(
  ...
  const isLeaf = isAddFuelVendorFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getAddFuelVendorFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampAddFuelVendorFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampAddFuelVendorFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampAddFuelVendorFormColSpan(
...
export function toggleAddFuelVendorFormActive(nodes = [], nodeId) {
  const nextTree = cloneAddFuelVendorFormTree(nodes);
  const existing = findAddFuelVendorFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleAddFuelVendorFormHidden(nodes = [], nodeId) {
  const nextTree = cloneAddFuelVendorFormTree(nodes);
  const existing = findAddFuelVendorFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteAddFuelVendorFormNode(nodes = [], nodeId) {
  const existing = findAddFuelVendorFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneAddFuelVendorFormTree(nodes));
}

export function reorderAddFuelVendorFormNode(nodes = [], nodeId, direction) {
  const existing = findAddFuelVendorFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneAddFuelVendorFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderAddFuelVendorFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findAddFuelVendorFormNodeById(nodes, sourceId);
...
export function getAddFuelVendorFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Add Fuel Vendor. */
export function leafToAddFuelVendorField(leaf, sectionId) {
