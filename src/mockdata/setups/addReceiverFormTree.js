/**
 * Add Receiver Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  ADD_RECEIVER_FORM_GROUPS,
  ADD_RECEIVER_FORM_SECTIONS,
...
export const ADD_RECEIVER_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const ADD_RECEIVER_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampAddReceiverFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampAddReceiverFormColSpan(value, columns = 2) {
  const max = clampAddReceiverFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampAddReceiverFormColumns(columns);
  // Add Receiver defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampAddReceiverFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const ADD_RECEIVER_FORM_LEVEL_1 = ADD_RECEIVER_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampAddReceiverFormColSpan(
 ...
 */
export function buildInitialAddReceiverFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return ADD_RECEIVER_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampAddReceiverFormColumns(section.columns ?? 2);
    const groups = ADD_RECEIVER_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = ADD_RECEIVER_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampAddReceiverFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = ADD_RECEIVER_FORM_FIELD_CATALOG.filter(
...
export function cloneAddReceiverFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneAddReceiverFormTree(node.children || []),
  }));
}

export function isAddReceiverFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isAddReceiverFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenAddReceiverFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenAddReceiverFormTree(node.children, acc);
  });
  return acc;
}

export function findAddReceiverFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findAddReceiverFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findAddReceiverFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findAddReceiverFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getAddReceiverFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findAddReceiverFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findAddReceiverFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getAddReceiverFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findAddReceiverFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findAddReceiverFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getAddReceiverFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getAddReceiverFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveAddReceiverFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isAddReceiverFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveAddReceiverFormLeaves(child));
}

export function collectAllActiveAddReceiverFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveAddReceiverFormLeaves(node));
}
...
export function filterAddReceiverFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getAddReceiverFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenAddReceiverFormTree(nodes)
    .filter((node) => {
      if (isAddReceiverFormValueField(node)) return false;
...
export function addAddReceiverFormNode(nodes = [], payload) {
  const nextTree = cloneAddReceiverFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findAddReceiverFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isAddReceiverFormValueField(parent)) {
  ...
  if (findAddReceiverFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getAddReceiverFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampAddReceiverFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampAddReceiverFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampAddReceiverFormColSpan(
...
export function updateAddReceiverFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneAddReceiverFormTree(nodes);
  const existing = findAddReceiverFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampAddReceiverFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampAddReceiverFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findAddReceiverFormParentNode(nextTree, nodeId);
    const sectionColumns = clampAddReceiverFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampAddReceiverFormColSpan(
  ...
  const isLeaf = isAddReceiverFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getAddReceiverFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampAddReceiverFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampAddReceiverFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampAddReceiverFormColSpan(
...
export function toggleAddReceiverFormActive(nodes = [], nodeId) {
  const nextTree = cloneAddReceiverFormTree(nodes);
  const existing = findAddReceiverFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleAddReceiverFormHidden(nodes = [], nodeId) {
  const nextTree = cloneAddReceiverFormTree(nodes);
  const existing = findAddReceiverFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteAddReceiverFormNode(nodes = [], nodeId) {
  const existing = findAddReceiverFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneAddReceiverFormTree(nodes));
}

export function reorderAddReceiverFormNode(nodes = [], nodeId, direction) {
  const existing = findAddReceiverFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneAddReceiverFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderAddReceiverFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findAddReceiverFormNodeById(nodes, sourceId);
...
export function getAddReceiverFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Add Receiver. */
