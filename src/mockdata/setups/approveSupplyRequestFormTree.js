/**
 * Approve Supply Request Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  APPROVE_SUPPLY_REQUEST_FORM_GROUPS,
  APPROVE_SUPPLY_REQUEST_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./approveSupplyRequestFormFields";
import {
...
export const APPROVE_SUPPLY_REQUEST_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const APPROVE_SUPPLY_REQUEST_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampApproveSupplyRequestFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampApproveSupplyRequestFormColSpan(value, columns = 2) {
  const max = clampApproveSupplyRequestFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampApproveSupplyRequestFormColumns(columns);
  // Approve Supply Request defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampApproveSupplyRequestFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const APPROVE_SUPPLY_REQUEST_FORM_LEVEL_1 = APPROVE_SUPPLY_REQUEST_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampApproveSupplyRequestFormColSpan(
 ...
 */
export function buildInitialApproveSupplyRequestFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return APPROVE_SUPPLY_REQUEST_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampApproveSupplyRequestFormColumns(section.columns ?? 2);
    const groups = APPROVE_SUPPLY_REQUEST_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = APPROVE_SUPPLY_REQUEST_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampApproveSupplyRequestFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = APPROVE_SUPPLY_REQUEST_FORM_FIELD_CATALOG.filter(
...
export function cloneApproveSupplyRequestFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneApproveSupplyRequestFormTree(node.children || []),
  }));
}

export function isApproveSupplyRequestFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isApproveSupplyRequestFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenApproveSupplyRequestFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenApproveSupplyRequestFormTree(node.children, acc);
  });
  return acc;
}

export function findApproveSupplyRequestFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findApproveSupplyRequestFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findApproveSupplyRequestFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findApproveSupplyRequestFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getApproveSupplyRequestFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findApproveSupplyRequestFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findApproveSupplyRequestFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getApproveSupplyRequestFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findApproveSupplyRequestFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findApproveSupplyRequestFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getApproveSupplyRequestFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getApproveSupplyRequestFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveApproveSupplyRequestFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isApproveSupplyRequestFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveApproveSupplyRequestFormLeaves(child));
}

export function collectAllActiveApproveSupplyRequestFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveApproveSupplyRequestFormLeaves(node));
}
...
export function filterApproveSupplyRequestFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getApproveSupplyRequestFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenApproveSupplyRequestFormTree(nodes)
    .filter((node) => {
      if (isApproveSupplyRequestFormValueField(node)) return false;
...
export function addApproveSupplyRequestFormNode(nodes = [], payload) {
  const nextTree = cloneApproveSupplyRequestFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findApproveSupplyRequestFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isApproveSupplyRequestFormValueField(parent)) {
  ...
  if (findApproveSupplyRequestFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getApproveSupplyRequestFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampApproveSupplyRequestFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampApproveSupplyRequestFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampApproveSupplyRequestFormColSpan(
...
export function updateApproveSupplyRequestFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneApproveSupplyRequestFormTree(nodes);
  const existing = findApproveSupplyRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampApproveSupplyRequestFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampApproveSupplyRequestFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findApproveSupplyRequestFormParentNode(nextTree, nodeId);
    const sectionColumns = clampApproveSupplyRequestFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampApproveSupplyRequestFormColSpan(
  ...
  const isLeaf = isApproveSupplyRequestFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getApproveSupplyRequestFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampApproveSupplyRequestFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampApproveSupplyRequestFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampApproveSupplyRequestFormColSpan(
...
export function toggleApproveSupplyRequestFormActive(nodes = [], nodeId) {
  const nextTree = cloneApproveSupplyRequestFormTree(nodes);
  const existing = findApproveSupplyRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleApproveSupplyRequestFormHidden(nodes = [], nodeId) {
  const nextTree = cloneApproveSupplyRequestFormTree(nodes);
  const existing = findApproveSupplyRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteApproveSupplyRequestFormNode(nodes = [], nodeId) {
  const existing = findApproveSupplyRequestFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneApproveSupplyRequestFormTree(nodes));
}

export function reorderApproveSupplyRequestFormNode(nodes = [], nodeId, direction) {
  const existing = findApproveSupplyRequestFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneApproveSupplyRequestFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderApproveSupplyRequestFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findApproveSupplyRequestFormNodeById(nodes, sourceId);
...
export function getApproveSupplyRequestFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Approve Supply Request. */
export function leafToApproveSupplyRequestField(leaf, sectionId) {
