/**
 * Assignment form hierarchy — fixed Level 1 sections with open-ended nesting.
...
export const ASSIGNMENT_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const ASSIGNMENT_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampAssignmentFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampAssignmentFormColSpan(value, columns = 2) {
  const max = clampAssignmentFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampAssignmentFormColumns(columns);
  // Assignment modals default to full width; half-width fields set colSpan explicitly.
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampAssignmentFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const ASSIGNMENT_FORM_LEVEL_1 = ASSIGNMENT_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
 ...
 * Default visibility matches the previous Default Assignment Form setup.
 */
export function buildInitialAssignmentFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return ASSIGNMENT_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const fields = ASSIGNMENT_FORM_FIELD_CATALOG.filter(
      (field) => field.sectionId === section.id,
    );
    const sectionColumns = clampAssignmentFormColumns(section.columns ?? 2);
    ...
    return {
      ...
      children: fields.map((field, fieldIndex) => {
        ...
        return {
          ...
          columns: null,
          colSpan: clampAssignmentFormColSpan(
...
export function cloneAssignmentFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneAssignmentFormTree(node.children || []),
  }));
}

export function isAssignmentFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isAssignmentFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenAssignmentFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenAssignmentFormTree(node.children, acc);
  });
  return acc;
}

export function findAssignmentFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findAssignmentFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findAssignmentFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findAssignmentFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getAssignmentFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findAssignmentFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findAssignmentFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getAssignmentFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findAssignmentFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findAssignmentFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getAssignmentFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getAssignmentFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveAssignmentFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isAssignmentFormValueField(node)) {
    return [node];
  }
  return (node.children || []).flatMap((child) => collectActiveAssignmentFormLeaves(child));
}

export function collectAllActiveAssignmentFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveAssignmentFormLeaves(node));
}
...
export function filterAssignmentFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getAssignmentFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenAssignmentFormTree(nodes)
    .filter((node) => {
      if (isAssignmentFormValueField(node)) return false;
...
export function addAssignmentFormNode(nodes = [], payload) {
  const nextTree = cloneAssignmentFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findAssignmentFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isAssignmentFormValueField(parent)) {
  ...
  if (findAssignmentFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getAssignmentFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampAssignmentFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampAssignmentFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampAssignmentFormColSpan(
...
export function updateAssignmentFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneAssignmentFormTree(nodes);
  const existing = findAssignmentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (name/description/active stay fixed).
  if (existing.isLocked) {
    const columns = clampAssignmentFormColumns(payload.columns ?? existing.columns ?? 2);
  ...
  if (existing.isDefaultLocked) {
    const l1 = getAssignmentFormLevel1Ancestor(nextTree, nodeId);
    const sectionColumns = clampAssignmentFormColumns(l1?.columns ?? 2);
  ...
  const isLeaf = isAssignmentFormLeaf(existing);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampAssignmentFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampAssignmentFormColSpan(
...
export function toggleAssignmentFormActive(nodes = [], nodeId) {
  const nextTree = cloneAssignmentFormTree(nodes);
  const existing = findAssignmentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleAssignmentFormHidden(nodes = [], nodeId) {
  const nextTree = cloneAssignmentFormTree(nodes);
  const existing = findAssignmentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteAssignmentFormNode(nodes = [], nodeId) {
  const existing = findAssignmentFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneAssignmentFormTree(nodes));
}

export function reorderAssignmentFormNode(nodes = [], nodeId, direction) {
  const { nodes: next, moved } = reorderTreeSibling(
    cloneAssignmentFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderAssignmentFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
...
export function getAssignmentFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the legacy field shape used by AssignmentFormWizard. */
export function leafToAssignmentField(leaf, sectionId) {
