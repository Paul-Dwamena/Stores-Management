/**
 * Compliance document form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  COMPLIANCE_DOCUMENT_FORM_FIELD_CATALOG,
  COMPLIANCE_DOCUMENT_FORM_GROUPS,
  COMPLIANCE_DOCUMENT_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./complianceDocumentFormFields";
import {
...
export const COMPLIANCE_DOCUMENT_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const COMPLIANCE_DOCUMENT_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampComplianceDocumentFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampComplianceDocumentFormColSpan(value, columns = 2) {
  const max = clampComplianceDocumentFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampComplianceDocumentFormColumns(columns);
  // Upload Compliance Documents defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampComplianceDocumentFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const COMPLIANCE_DOCUMENT_FORM_LEVEL_1 = COMPLIANCE_DOCUMENT_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampComplianceDocumentFormColSpan(
...
/**
 * Seed hierarchy: Main holds document file, name, linked entity, and expiry fields.
 */
export function buildInitialComplianceDocumentFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return COMPLIANCE_DOCUMENT_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampComplianceDocumentFormColumns(section.columns ?? 2);
    const groups = COMPLIANCE_DOCUMENT_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = COMPLIANCE_DOCUMENT_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampComplianceDocumentFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = COMPLIANCE_DOCUMENT_FORM_FIELD_CATALOG.filter(
...
export function cloneComplianceDocumentFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneComplianceDocumentFormTree(node.children || []),
  }));
}

export function isComplianceDocumentFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isComplianceDocumentFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenComplianceDocumentFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenComplianceDocumentFormTree(node.children, acc);
  });
  return acc;
}

export function findComplianceDocumentFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findComplianceDocumentFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findComplianceDocumentFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findComplianceDocumentFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getComplianceDocumentFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findComplianceDocumentFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findComplianceDocumentFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getComplianceDocumentFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findComplianceDocumentFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findComplianceDocumentFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getComplianceDocumentFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getComplianceDocumentFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveComplianceDocumentFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isComplianceDocumentFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveComplianceDocumentFormLeaves(child));
}

export function collectAllActiveComplianceDocumentFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveComplianceDocumentFormLeaves(node));
}
...
export function filterComplianceDocumentFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getComplianceDocumentFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenComplianceDocumentFormTree(nodes)
    .filter((node) => {
      if (isComplianceDocumentFormValueField(node)) return false;
...
export function addComplianceDocumentFormNode(nodes = [], payload) {
  const nextTree = cloneComplianceDocumentFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findComplianceDocumentFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isComplianceDocumentFormValueField(parent)) {
  ...
  if (findComplianceDocumentFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getComplianceDocumentFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampComplianceDocumentFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampComplianceDocumentFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampComplianceDocumentFormColSpan(
...
export function updateComplianceDocumentFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneComplianceDocumentFormTree(nodes);
  const existing = findComplianceDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampComplianceDocumentFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampComplianceDocumentFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findComplianceDocumentFormParentNode(nextTree, nodeId);
    const sectionColumns = clampComplianceDocumentFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampComplianceDocumentFormColSpan(
  ...
  const isLeaf = isComplianceDocumentFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getComplianceDocumentFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampComplianceDocumentFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampComplianceDocumentFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampComplianceDocumentFormColSpan(
...
export function toggleComplianceDocumentFormActive(nodes = [], nodeId) {
  const nextTree = cloneComplianceDocumentFormTree(nodes);
  const existing = findComplianceDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleComplianceDocumentFormHidden(nodes = [], nodeId) {
  const nextTree = cloneComplianceDocumentFormTree(nodes);
  const existing = findComplianceDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteComplianceDocumentFormNode(nodes = [], nodeId) {
  const existing = findComplianceDocumentFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneComplianceDocumentFormTree(nodes));
}

export function reorderComplianceDocumentFormNode(nodes = [], nodeId, direction) {
  const existing = findComplianceDocumentFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneComplianceDocumentFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderComplianceDocumentFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findComplianceDocumentFormNodeById(nodes, sourceId);
...
export function getComplianceDocumentFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Upload Compliance Documents. */
export function leafToComplianceDocumentField(leaf, sectionId) {
