/**
 * Create Service Program Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  CREATE_SERVICE_PROGRAM_FORM_FIELD_CATALOG,
  CREATE_SERVICE_PROGRAM_FORM_GROUPS,
  CREATE_SERVICE_PROGRAM_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./createServiceProgramFormFields";
import {
...
export const CREATE_SERVICE_PROGRAM_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const CREATE_SERVICE_PROGRAM_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampCreateServiceProgramFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampCreateServiceProgramFormColSpan(value, columns = 2) {
  const max = clampCreateServiceProgramFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampCreateServiceProgramFormColumns(columns);
  // Create Service Program defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampCreateServiceProgramFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const CREATE_SERVICE_PROGRAM_FORM_LEVEL_1 = CREATE_SERVICE_PROGRAM_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampCreateServiceProgramFormColSpan(
 ...
 */
export function buildInitialCreateServiceProgramFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return CREATE_SERVICE_PROGRAM_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampCreateServiceProgramFormColumns(section.columns ?? 2);
    const groups = CREATE_SERVICE_PROGRAM_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = CREATE_SERVICE_PROGRAM_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampCreateServiceProgramFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = CREATE_SERVICE_PROGRAM_FORM_FIELD_CATALOG.filter(
...
export function cloneCreateServiceProgramFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneCreateServiceProgramFormTree(node.children || []),
  }));
}

export function isCreateServiceProgramFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isCreateServiceProgramFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenCreateServiceProgramFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenCreateServiceProgramFormTree(node.children, acc);
  });
  return acc;
}

export function findCreateServiceProgramFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findCreateServiceProgramFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findCreateServiceProgramFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findCreateServiceProgramFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getCreateServiceProgramFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findCreateServiceProgramFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findCreateServiceProgramFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getCreateServiceProgramFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findCreateServiceProgramFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findCreateServiceProgramFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getCreateServiceProgramFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getCreateServiceProgramFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveCreateServiceProgramFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isCreateServiceProgramFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveCreateServiceProgramFormLeaves(child));
}

export function collectAllActiveCreateServiceProgramFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveCreateServiceProgramFormLeaves(node));
}
...
export function filterCreateServiceProgramFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getCreateServiceProgramFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenCreateServiceProgramFormTree(nodes)
    .filter((node) => {
      if (isCreateServiceProgramFormValueField(node)) return false;
...
export function addCreateServiceProgramFormNode(nodes = [], payload) {
  const nextTree = cloneCreateServiceProgramFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findCreateServiceProgramFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isCreateServiceProgramFormValueField(parent)) {
  ...
  if (findCreateServiceProgramFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getCreateServiceProgramFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampCreateServiceProgramFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampCreateServiceProgramFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampCreateServiceProgramFormColSpan(
...
export function updateCreateServiceProgramFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneCreateServiceProgramFormTree(nodes);
  const existing = findCreateServiceProgramFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampCreateServiceProgramFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampCreateServiceProgramFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findCreateServiceProgramFormParentNode(nextTree, nodeId);
    const sectionColumns = clampCreateServiceProgramFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampCreateServiceProgramFormColSpan(
  ...
  const isLeaf = isCreateServiceProgramFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getCreateServiceProgramFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampCreateServiceProgramFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampCreateServiceProgramFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampCreateServiceProgramFormColSpan(
...
export function toggleCreateServiceProgramFormActive(nodes = [], nodeId) {
  const nextTree = cloneCreateServiceProgramFormTree(nodes);
  const existing = findCreateServiceProgramFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleCreateServiceProgramFormHidden(nodes = [], nodeId) {
  const nextTree = cloneCreateServiceProgramFormTree(nodes);
  const existing = findCreateServiceProgramFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteCreateServiceProgramFormNode(nodes = [], nodeId) {
  const existing = findCreateServiceProgramFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneCreateServiceProgramFormTree(nodes));
}

export function reorderCreateServiceProgramFormNode(nodes = [], nodeId, direction) {
  const existing = findCreateServiceProgramFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneCreateServiceProgramFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderCreateServiceProgramFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findCreateServiceProgramFormNodeById(nodes, sourceId);
...
export function getCreateServiceProgramFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Create Service Program. */
export function leafToCreateServiceProgramField(leaf, sectionId) {
