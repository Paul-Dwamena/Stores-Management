/**
 * Driver document form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  DRIVER_DOCUMENT_FORM_FIELD_CATALOG,
  DRIVER_DOCUMENT_FORM_GROUPS,
  DRIVER_DOCUMENT_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./driverDocumentFormFields";
import {
...
export const DRIVER_DOCUMENT_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const DRIVER_DOCUMENT_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampDriverDocumentFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampDriverDocumentFormColSpan(value, columns = 2) {
  const max = clampDriverDocumentFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampDriverDocumentFormColumns(columns);
  // Upload Driver Documents defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampDriverDocumentFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const DRIVER_DOCUMENT_FORM_LEVEL_1 = DRIVER_DOCUMENT_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampDriverDocumentFormColSpan(
...
/**
 * Seed hierarchy: Main holds document file, name, driver, and expiry fields.
 */
export function buildInitialDriverDocumentFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return DRIVER_DOCUMENT_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampDriverDocumentFormColumns(section.columns ?? 2);
    const groups = DRIVER_DOCUMENT_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = DRIVER_DOCUMENT_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampDriverDocumentFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = DRIVER_DOCUMENT_FORM_FIELD_CATALOG.filter(
...
export function cloneDriverDocumentFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneDriverDocumentFormTree(node.children || []),
  }));
}

export function isDriverDocumentFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isDriverDocumentFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenDriverDocumentFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenDriverDocumentFormTree(node.children, acc);
  });
  return acc;
}

export function findDriverDocumentFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findDriverDocumentFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findDriverDocumentFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findDriverDocumentFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getDriverDocumentFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findDriverDocumentFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findDriverDocumentFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getDriverDocumentFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findDriverDocumentFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findDriverDocumentFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getDriverDocumentFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getDriverDocumentFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveDriverDocumentFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isDriverDocumentFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveDriverDocumentFormLeaves(child));
}

export function collectAllActiveDriverDocumentFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveDriverDocumentFormLeaves(node));
}
...
export function filterDriverDocumentFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getDriverDocumentFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenDriverDocumentFormTree(nodes)
    .filter((node) => {
      if (isDriverDocumentFormValueField(node)) return false;
...
export function addDriverDocumentFormNode(nodes = [], payload) {
  const nextTree = cloneDriverDocumentFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findDriverDocumentFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isDriverDocumentFormValueField(parent)) {
  ...
  if (findDriverDocumentFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getDriverDocumentFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampDriverDocumentFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampDriverDocumentFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampDriverDocumentFormColSpan(
...
export function updateDriverDocumentFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneDriverDocumentFormTree(nodes);
  const existing = findDriverDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampDriverDocumentFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampDriverDocumentFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findDriverDocumentFormParentNode(nextTree, nodeId);
    const sectionColumns = clampDriverDocumentFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampDriverDocumentFormColSpan(
  ...
  const isLeaf = isDriverDocumentFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getDriverDocumentFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampDriverDocumentFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampDriverDocumentFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampDriverDocumentFormColSpan(
...
export function toggleDriverDocumentFormActive(nodes = [], nodeId) {
  const nextTree = cloneDriverDocumentFormTree(nodes);
  const existing = findDriverDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleDriverDocumentFormHidden(nodes = [], nodeId) {
  const nextTree = cloneDriverDocumentFormTree(nodes);
  const existing = findDriverDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteDriverDocumentFormNode(nodes = [], nodeId) {
  const existing = findDriverDocumentFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneDriverDocumentFormTree(nodes));
}

export function reorderDriverDocumentFormNode(nodes = [], nodeId, direction) {
  const existing = findDriverDocumentFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneDriverDocumentFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderDriverDocumentFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findDriverDocumentFormNodeById(nodes, sourceId);
...
export function getDriverDocumentFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Upload Driver Documents. */
export function leafToDriverDocumentField(leaf, sectionId) {
