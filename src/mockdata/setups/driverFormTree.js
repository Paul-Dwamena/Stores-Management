/**
 * Driver form hierarchy — fixed Level 1 sections with open-ended nesting.
...
import {
  ...
  DRIVER_FORM_FIELD_CATALOG,
  DRIVER_FORM_SECTIONS,
...
export const DRIVER_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const DRIVER_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampDriverFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampDriverFormColSpan(value, columns = 2) {
  const max = clampDriverFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  ...
  if (fieldType === "photo" || fieldTypeIsFile(fieldType)) return Math.min(2, max);
  return 1;
}
...
function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (Boolean(child?.key)) {
      return {
        ...child,
        colSpan: clampDriverFormColSpan(child.colSpan ?? 1, columns),
      };
    }
    return {
      ...child,
...
export const DRIVER_FORM_LEVEL_1 = DRIVER_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
 ...
 * Default visibility matches the previous Default Driver Form setup.
 */
export function buildInitialDriverFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return DRIVER_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const fields = DRIVER_FORM_FIELD_CATALOG.filter(
    ...
    return {
      ...
      children: fields.map((field, fieldIndex) => {
        ...
        return {
          ...
          columns: null,
          colSpan: clampDriverFormColSpan(
...
export function cloneDriverFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneDriverFormTree(node.children || []),
  }));
}

export function isDriverFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isDriverFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenDriverFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenDriverFormTree(node.children, acc);
  });
  return acc;
}

export function findDriverFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findDriverFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findDriverFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findDriverFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getDriverFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findDriverFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findDriverFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getDriverFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findDriverFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findDriverFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getDriverFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getDriverFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveDriverFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isDriverFormValueField(node)) {
    return [node];
  }
  return (node.children || []).flatMap((child) => collectActiveDriverFormLeaves(child));
}

export function collectAllActiveDriverFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveDriverFormLeaves(node));
}
...
export function filterDriverFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getDriverFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenDriverFormTree(nodes)
    .filter((node) => {
      if (isDriverFormValueField(node)) return false;
...
export function addDriverFormNode(nodes = [], payload) {
  const nextTree = cloneDriverFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findDriverFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isDriverFormValueField(parent)) {
  ...
  if (findDriverFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getDriverFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampDriverFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampDriverFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampDriverFormColSpan(
...
export function updateDriverFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneDriverFormTree(nodes);
  const existing = findDriverFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (name/description/active stay fixed).
  if (existing.isLocked) {
    const columns = clampDriverFormColumns(payload.columns ?? existing.columns ?? 2);
  ...
  if (existing.isDefaultLocked) {
    const l1 = getDriverFormLevel1Ancestor(nextTree, nodeId);
    const sectionColumns = clampDriverFormColumns(l1?.columns ?? 2);
  ...
  const isLeaf = isDriverFormLeaf(existing);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampDriverFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampDriverFormColSpan(
...
export function toggleDriverFormActive(nodes = [], nodeId) {
  const nextTree = cloneDriverFormTree(nodes);
  const existing = findDriverFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleDriverFormHidden(nodes = [], nodeId) {
  const nextTree = cloneDriverFormTree(nodes);
  const existing = findDriverFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteDriverFormNode(nodes = [], nodeId) {
  const existing = findDriverFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneDriverFormTree(nodes));
}

export function reorderDriverFormNode(nodes = [], nodeId, direction) {
  const { nodes: next, moved } = reorderTreeSibling(
    cloneDriverFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderDriverFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
...
export function getDriverFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the legacy field shape used by DriverFormWizard. */
export function leafToDriverField(leaf, sectionId) {
