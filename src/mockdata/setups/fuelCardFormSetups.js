 ...
 * window-backed store so Link Fuel Card forms always see the latest config.
 */

import {
  FUEL_CARD_FORM_LEVEL_1,
  buildInitialFuelCardFormTree,
  clampFuelCardFormColumns,
  clampLeavesColSpan,
  cloneFuelCardFormTree,
  collectActiveFuelCardFormLeaves,
  findFuelCardFormNodeById,
  getFuelCardFormLevel1Ancestor,
  leafToFuelCardField,
} from "./fuelCardFormTree";
import { FUEL_CARD_FORM_SECTIONS, FUEL_CARD_FORM_FIELD_CATALOG } from "./fuelCardFormFields";
...
export const FUEL_CARD_FORM_SETUP_CHANGED_EVENT = "fleetly-fuel-card-form-setup-changed";

const treeStore = createSessionTreeStore({
  storageKey: "fleetly_fuel_card_form_tree",
  eventName: FUEL_CARD_FORM_SETUP_CHANGED_EVENT,
  windowKey: "__FLEETLY_FUEL_CARD_FORM_TREE__",
  getSeed: () => buildInitialFuelCardFormTree(),
  clone: cloneFuelCardFormTree,
});

const META_KEY = "fleetly_fuel_card_form_meta";
const TREE_SCHEMA_VERSION = 4;
const FUEL_CARD_FIELD_TYPE_TEST_LEVEL_ID = "field_type_preview";

function readMeta() {
  const fallback = {
    id: "fuelcf_001",
    name: "Default Link Fuel Card Form",
    isActive: true,
    updatedAt: null,
    treeSchemaVersion: 0,
  };
...
function ensureLevel1Sections(tree = []) {
  ...
  const missing = buildInitialFuelCardFormTree().filter(
...
function syncLevel1SectionMeta(tree = [], { applyCatalogColumns = false } = {}) {
  return tree.map((root) => {
    ...
    const meta = FUEL_CARD_FORM_SECTIONS.find((section) => section.id === root.id);
    if (!meta) return root;
    ...
    if (applyCatalogColumns && meta.columns != null) {
      const columns = clampFuelCardFormColumns(meta.columns);
      const children = (root.children || []).map((child) => {
        const catalog = FUEL_CARD_FORM_FIELD_CATALOG.find((field) => field.id === child.id);
...
function ensureTreeSchema() {
  ...
  if (version < 4) {
    tree = tree.filter((node) => node.id !== FUEL_CARD_FIELD_TYPE_TEST_LEVEL_ID);
  }
...
function buildVisibleFieldIdsBySection(tree = []) {
  return Object.fromEntries(
    tree.map((root) => [
      root.id,
      collectActiveFuelCardFormLeaves(root).map((leaf) => leaf.id),
    ]),
  );
}

export function getFuelCardFormSetup() {
  const meta = readMeta();
  const tree = getOrderedTree();
  return {
    id: meta.id || "fuelcf_001",
    name: meta.name || "Default Link Fuel Card Form",
...
export function getFuelCardFormTree(setup = getFuelCardFormSetup()) {
  return cloneFuelCardFormTree(setup.tree || getOrderedTree());
}

export function saveFuelCardFormSetup(payload = {}) {
  const now = new Date().toISOString();
  const current = getFuelCardFormSetup();
  const meta = {
    id: current.id,
  ...
  if (payload.tree) {
    ...
    window.dispatchEvent(new CustomEvent(FUEL_CARD_FORM_SETUP_CHANGED_EVENT));
  }
  return getFuelCardFormSetup();
}

export function saveFuelCardFormTree(tree) {
  return saveFuelCardFormSetup({ tree });
}
...
export function getFuelCardFormFields(setup = getFuelCardFormSetup()) {
  const tree = setup.tree || [];
  return flattenIncludingInactive(tree)
    .filter((node) => node.key)
    .map((leaf) => {
      const l1 = getFuelCardFormLevel1Ancestor(tree, leaf.id);
      return leafToFuelCardField(leaf, l1?.id || null);
    });
}

export function getFuelCardFormFieldsBySection(sectionId, setup = getFuelCardFormSetup()) {
  return getFuelCardFormFields(setup).filter((field) => field.sectionId === sectionId);
}

export function getVisibleFuelCardFieldsForSection(
  sectionId,
  setup = getFuelCardFormSetup(),
) {
  const root = findFuelCardFormNodeById(setup.tree || [], sectionId);
  if (!root || root.isActive === false) return [];
  return collectActiveFuelCardFormLeaves(root).map((leaf) =>
    leafToFuelCardField(leaf, sectionId),
  );
}
 ...
 */
export function getActiveFuelCardFormSections(setup = getFuelCardFormSetup()) {
  const tree = sortTreeSiblings(setup?.tree || getOrderedTree());

  return tree
    .map((root, index) => {
      if (!root || root.isActive === false) return null;
      const fields = collectActiveFuelCardFormLeaves(root).map((leaf) =>
        leafToFuelCardField(leaf, root.id),
      );
      if (!fields.length) return null;
      const sectionMeta = FUEL_CARD_FORM_SECTIONS.find((section) => section.id === root.id);
      return {
        id: root.id,
        label: root.name,
        description: root.description || sectionMeta?.description || "",
        columns: clampFuelCardFormColumns(root.columns ?? 2),
...
export function resetFuelCardFormSetup() {
  writeMeta({
    id: "fuelcf_001",
    name: "Default Link Fuel Card Form",
    isActive: true,
