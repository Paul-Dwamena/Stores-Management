...
import {
  ...
  FUEL_CARD_FORM_FIELD_CATALOG,
  FUEL_CARD_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./fuelCardFormFields";
import {
...
export const FUEL_CARD_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const FUEL_CARD_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampFuelCardFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampFuelCardFormColSpan(value, columns = 2) {
  const max = clampFuelCardFormColumns(columns);
  const n = Number(value);
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampFuelCardFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const FUEL_CARD_FORM_LEVEL_1 = FUEL_CARD_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
 ...
 */
export function buildInitialFuelCardFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return FUEL_CARD_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const fields = FUEL_CARD_FORM_FIELD_CATALOG.filter(
      (field) => field.sectionId === section.id,
    );
    const sectionColumns = clampFuelCardFormColumns(section.columns ?? 2);
    ...
    return {
      ...
      children: fields.map((field, fieldIndex) => {
        ...
        return {
          ...
          columns: null,
          colSpan: clampFuelCardFormColSpan(
...
export function cloneFuelCardFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneFuelCardFormTree(node.children || []),
  }));
}

export function isFuelCardFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isFuelCardFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenFuelCardFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenFuelCardFormTree(node.children, acc);
  });
  return acc;
}

export function findFuelCardFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findFuelCardFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findFuelCardFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findFuelCardFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getFuelCardFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findFuelCardFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findFuelCardFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getFuelCardFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findFuelCardFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findFuelCardFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getFuelCardFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getFuelCardFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveFuelCardFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isFuelCardFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveFuelCardFormLeaves(child));
}

export function collectAllActiveFuelCardFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveFuelCardFormLeaves(node));
}
...
export function filterFuelCardFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getFuelCardFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenFuelCardFormTree(nodes)
    .filter((node) => {
      if (isFuelCardFormValueField(node)) return false;
...
export function addFuelCardFormNode(nodes = [], payload) {
  const nextTree = cloneFuelCardFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findFuelCardFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isFuelCardFormValueField(parent)) {
  ...
  if (findFuelCardFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getFuelCardFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampFuelCardFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampFuelCardFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampFuelCardFormColSpan(
...
export function updateFuelCardFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneFuelCardFormTree(nodes);
  const existing = findFuelCardFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (name/description/active stay fixed).
  if (existing.isLocked) {
    const columns = clampFuelCardFormColumns(payload.columns ?? existing.columns ?? 2);
  ...
  if (existing.isDefaultLocked) {
    const l1 = getFuelCardFormLevel1Ancestor(nextTree, nodeId);
    const sectionColumns = clampFuelCardFormColumns(l1?.columns ?? 2);
  ...
  const isLeaf = isFuelCardFormLeaf(existing);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampFuelCardFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampFuelCardFormColSpan(
...
export function toggleFuelCardFormActive(nodes = [], nodeId) {
  const nextTree = cloneFuelCardFormTree(nodes);
  const existing = findFuelCardFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleFuelCardFormHidden(nodes = [], nodeId) {
  const nextTree = cloneFuelCardFormTree(nodes);
  const existing = findFuelCardFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteFuelCardFormNode(nodes = [], nodeId) {
  const existing = findFuelCardFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneFuelCardFormTree(nodes));
}

export function reorderFuelCardFormNode(nodes = [], nodeId, direction) {
  const { nodes: next, moved } = reorderTreeSibling(
    cloneFuelCardFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderFuelCardFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
...
export function getFuelCardFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the legacy field shape used by FuelCardFormWizard. */
export function leafToFuelCardField(leaf, sectionId) {
