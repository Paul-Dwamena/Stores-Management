...
import {
  ...
  FUEL_REQUEST_FORM_FIELD_CATALOG,
  FUEL_REQUEST_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./fuelRequestFormFields";
import {
...
export const FUEL_REQUEST_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const FUEL_REQUEST_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampFuelRequestFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampFuelRequestFormColSpan(value, columns = 2) {
  const max = clampFuelRequestFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampFuelRequestFormColumns(columns);
  // Fuel Entry defaults new fields to full row width (litres/amount/supply date stay half in catalog).
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampFuelRequestFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const FUEL_REQUEST_FORM_LEVEL_1 = FUEL_REQUEST_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
 ...
 */
export function buildInitialFuelRequestFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return FUEL_REQUEST_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const fields = FUEL_REQUEST_FORM_FIELD_CATALOG.filter(
      (field) => field.sectionId === section.id,
    );
    const sectionColumns = clampFuelRequestFormColumns(section.columns ?? 2);
    ...
    return {
      ...
      children: fields.map((field, fieldIndex) => {
        ...
        return {
          ...
          columns: null,
          colSpan: clampFuelRequestFormColSpan(
...
export function cloneFuelRequestFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneFuelRequestFormTree(node.children || []),
  }));
}

export function isFuelRequestFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isFuelRequestFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenFuelRequestFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenFuelRequestFormTree(node.children, acc);
  });
  return acc;
}

export function findFuelRequestFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findFuelRequestFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findFuelRequestFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findFuelRequestFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getFuelRequestFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findFuelRequestFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findFuelRequestFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getFuelRequestFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findFuelRequestFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findFuelRequestFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getFuelRequestFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getFuelRequestFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveFuelRequestFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isFuelRequestFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveFuelRequestFormLeaves(child));
}

export function collectAllActiveFuelRequestFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveFuelRequestFormLeaves(node));
}
...
export function filterFuelRequestFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getFuelRequestFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenFuelRequestFormTree(nodes)
    .filter((node) => {
      if (isFuelRequestFormValueField(node)) return false;
...
export function addFuelRequestFormNode(nodes = [], payload) {
  const nextTree = cloneFuelRequestFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findFuelRequestFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isFuelRequestFormValueField(parent)) {
  ...
  if (findFuelRequestFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getFuelRequestFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampFuelRequestFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampFuelRequestFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampFuelRequestFormColSpan(
...
export function updateFuelRequestFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneFuelRequestFormTree(nodes);
  const existing = findFuelRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (name/description/active stay fixed).
  if (existing.isLocked) {
    const columns = clampFuelRequestFormColumns(payload.columns ?? existing.columns ?? 2);
  ...
  if (existing.isDefaultLocked) {
    const l1 = getFuelRequestFormLevel1Ancestor(nextTree, nodeId);
    const sectionColumns = clampFuelRequestFormColumns(l1?.columns ?? 2);
  ...
  const isLeaf = isFuelRequestFormLeaf(existing);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampFuelRequestFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampFuelRequestFormColSpan(
...
export function toggleFuelRequestFormActive(nodes = [], nodeId) {
  const nextTree = cloneFuelRequestFormTree(nodes);
  const existing = findFuelRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleFuelRequestFormHidden(nodes = [], nodeId) {
  const nextTree = cloneFuelRequestFormTree(nodes);
  const existing = findFuelRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteFuelRequestFormNode(nodes = [], nodeId) {
  const existing = findFuelRequestFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneFuelRequestFormTree(nodes));
}

export function reorderFuelRequestFormNode(nodes = [], nodeId, direction) {
  const { nodes: next, moved } = reorderTreeSibling(
    cloneFuelRequestFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderFuelRequestFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
...
export function getFuelRequestFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the legacy field shape used by FuelRequestFormWizard. */
export function leafToFuelRequestField(leaf, sectionId) {
