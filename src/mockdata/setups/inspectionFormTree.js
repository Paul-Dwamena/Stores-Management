/**
 * Inspection form hierarchy — fixed Level 1 sections with open-ended nesting.
...
import {
  ...
  INSPECTION_FORM_FIELD_CATALOG,
  INSPECTION_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./inspectionFormFields";
import {
...
export const INSPECTION_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const INSPECTION_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampInspectionFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampInspectionFormColSpan(value, columns = 2) {
  const max = clampInspectionFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampInspectionFormColumns(columns);
  // Inspection form fields default to full width; half-width fields set colSpan explicitly.
  if (fieldType === "photo" || fieldType === "image") return Math.min(2, max);
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampInspectionFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const INSPECTION_FORM_LEVEL_1 = INSPECTION_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
 ...
 * Default visibility matches the Default Inspection Form setup.
 */
export function buildInitialInspectionFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return INSPECTION_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const fields = INSPECTION_FORM_FIELD_CATALOG.filter(
      (field) => field.sectionId === section.id,
    );
    const sectionColumns = clampInspectionFormColumns(section.columns ?? 2);
    ...
    return {
      ...
      children: fields.map((field, fieldIndex) => {
        ...
        return {
          ...
          columns: null,
          colSpan: clampInspectionFormColSpan(
...
export function cloneInspectionFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneInspectionFormTree(node.children || []),
  }));
}

export function isInspectionFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isInspectionFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenInspectionFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenInspectionFormTree(node.children, acc);
  });
  return acc;
}

export function findInspectionFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findInspectionFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findInspectionFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findInspectionFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getInspectionFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findInspectionFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findInspectionFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getInspectionFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findInspectionFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findInspectionFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getInspectionFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getInspectionFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveInspectionFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isInspectionFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveInspectionFormLeaves(child));
}

export function collectAllActiveInspectionFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveInspectionFormLeaves(node));
}
...
export function filterInspectionFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getInspectionFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenInspectionFormTree(nodes)
    .filter((node) => {
      if (isInspectionFormValueField(node)) return false;
...
export function addInspectionFormNode(nodes = [], payload) {
  const nextTree = cloneInspectionFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findInspectionFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isInspectionFormValueField(parent)) {
  ...
  if (findInspectionFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getInspectionFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampInspectionFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampInspectionFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampInspectionFormColSpan(
...
export function updateInspectionFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneInspectionFormTree(nodes);
  const existing = findInspectionFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (name/description/active stay fixed).
  if (existing.isLocked) {
    const columns = clampInspectionFormColumns(payload.columns ?? existing.columns ?? 2);
  ...
  if (existing.isDefaultLocked) {
    const l1 = getInspectionFormLevel1Ancestor(nextTree, nodeId);
    const sectionColumns = clampInspectionFormColumns(l1?.columns ?? 2);
  ...
  const isLeaf = isInspectionFormLeaf(existing);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampInspectionFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampInspectionFormColSpan(
...
export function toggleInspectionFormActive(nodes = [], nodeId) {
  const nextTree = cloneInspectionFormTree(nodes);
  const existing = findInspectionFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleInspectionFormHidden(nodes = [], nodeId) {
  const nextTree = cloneInspectionFormTree(nodes);
  const existing = findInspectionFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteInspectionFormNode(nodes = [], nodeId) {
  const existing = findInspectionFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneInspectionFormTree(nodes));
}

export function reorderInspectionFormNode(nodes = [], nodeId, direction) {
  const { nodes: next, moved } = reorderTreeSibling(
    cloneInspectionFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderInspectionFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
...
export function getInspectionFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the legacy field shape used by InspectionFormWizard. */
export function leafToInspectionField(leaf, sectionId) {
