/**
 * Insurance claim form hierarchy — fixed Level 1 wizard steps.
 * Declaration & Signature nests Declaration and Signature folders; other steps
 * hold leaf fields directly. L1 order is not reorderable.
 */

import { moveTreeSiblingRelative, reorderTreeSibling } from "../../utils/treeReorder";
import {
  DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION,
  INSURANCE_CLAIM_FORM_FIELD_CATALOG,
  INSURANCE_CLAIM_FORM_GROUPS,
  INSURANCE_CLAIM_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./insuranceClaimFormFields";
import {
...
export const INSURANCE_CLAIM_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const INSURANCE_CLAIM_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampInsuranceClaimFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampInsuranceClaimFormColSpan(value, columns = 2) {
  const max = clampInsuranceClaimFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampInsuranceClaimFormColumns(columns);
  // File New Claim defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampInsuranceClaimFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const INSURANCE_CLAIM_FORM_LEVEL_1 = INSURANCE_CLAIM_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampInsuranceClaimFormColSpan(
      field.colSpan ?? defaultColSpanForFieldType(fieldType, sectionColumns),
      sectionColumns,
    ),
    acceptedFileTypes: fieldTypeIsFile(fieldType)
...
/**
 * Seed hierarchy: Incident Details and Supporting Documents have leaves under L1.
 * Declaration & Signature has locked Declaration / Signature folders.
 */
export function buildInitialInsuranceClaimFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return INSURANCE_CLAIM_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampInsuranceClaimFormColumns(section.columns ?? 2);
    const groups = INSURANCE_CLAIM_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const children = groups.length
      ? groups.map((group, groupIndex) => {
          const groupColumns = clampInsuranceClaimFormColumns(group.columns ?? sectionColumns);
          const groupNodeId = `${section.id}/${group.id}`;
          const fields = INSURANCE_CLAIM_FORM_FIELD_CATALOG.filter(
...
export function cloneInsuranceClaimFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneInsuranceClaimFormTree(node.children || []),
  }));
}

export function isInsuranceClaimFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isInsuranceClaimFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenInsuranceClaimFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenInsuranceClaimFormTree(node.children, acc);
  });
  return acc;
}

export function findInsuranceClaimFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findInsuranceClaimFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findInsuranceClaimFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findInsuranceClaimFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getInsuranceClaimFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findInsuranceClaimFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findInsuranceClaimFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getInsuranceClaimFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findInsuranceClaimFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findInsuranceClaimFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getInsuranceClaimFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getInsuranceClaimFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveInsuranceClaimFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isInsuranceClaimFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveInsuranceClaimFormLeaves(child));
}

export function collectAllActiveInsuranceClaimFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveInsuranceClaimFormLeaves(node));
}
...
export function filterInsuranceClaimFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getInsuranceClaimFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenInsuranceClaimFormTree(nodes)
    .filter((node) => {
      if (isInsuranceClaimFormValueField(node)) return false;
...
export function addInsuranceClaimFormNode(nodes = [], payload) {
  const nextTree = cloneInsuranceClaimFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findInsuranceClaimFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isInsuranceClaimFormValueField(parent)) {
  ...
  if (findInsuranceClaimFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getInsuranceClaimFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampInsuranceClaimFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampInsuranceClaimFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampInsuranceClaimFormColSpan(
          payload.colSpan ?? defaultColSpanForFieldType(fieldType, sectionColumns),
          sectionColumns,
        )
      : null,
    acceptedFileTypes: isValueField && fieldTypeIsFile(fieldType)
...
export function updateInsuranceClaimFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneInsuranceClaimFormTree(nodes);
  const existing = findInsuranceClaimFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampInsuranceClaimFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampInsuranceClaimFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findInsuranceClaimFormParentNode(nextTree, nodeId);
    const sectionColumns = clampInsuranceClaimFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampInsuranceClaimFormColSpan(
  ...
  const isLeaf = isInsuranceClaimFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getInsuranceClaimFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampInsuranceClaimFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampInsuranceClaimFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampInsuranceClaimFormColSpan(
...
export function toggleInsuranceClaimFormActive(nodes = [], nodeId) {
  const nextTree = cloneInsuranceClaimFormTree(nodes);
  const existing = findInsuranceClaimFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleInsuranceClaimFormHidden(nodes = [], nodeId) {
  const nextTree = cloneInsuranceClaimFormTree(nodes);
  const existing = findInsuranceClaimFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteInsuranceClaimFormNode(nodes = [], nodeId) {
  const existing = findInsuranceClaimFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneInsuranceClaimFormTree(nodes));
}

export function reorderInsuranceClaimFormNode(nodes = [], nodeId, direction) {
  const existing = findInsuranceClaimFormNodeById(nodes, nodeId);
  if (existing?.isLocked || existing?.level === 1) {
    throw new Error(
      "Wizard steps are fixed in order: Incident Details → Supporting Documents → Declaration & Signature.",
    );
  }
  const { nodes: next, moved } = reorderTreeSibling(
    cloneInsuranceClaimFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderInsuranceClaimFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findInsuranceClaimFormNodeById(nodes, sourceId);
...
export function getInsuranceClaimFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the legacy field shape used by InsuranceClaimFormWizard. */
export function leafToInsuranceClaimField(leaf, sectionId) {
  const rawType = leaf.fieldType || "text";
  const fieldType = fieldTypeIsFile(rawType) ? "file" : rawType;
