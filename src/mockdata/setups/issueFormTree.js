/**
 * Issue form hierarchy — fixed Level 1 sections with open-ended nesting.
...
export const ISSUE_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const ISSUE_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampIssueFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampIssueFormColSpan(value, columns = 2) {
  const max = clampIssueFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampIssueFormColumns(columns);
  // New fields default to one column; Vehicle / Component / Description set FULL in the catalog.
  if (fieldType === "photo" || fieldType === "image" || fieldType === "textarea") {
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampIssueFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const ISSUE_FORM_LEVEL_1 = ISSUE_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
 ...
 * Default visibility matches the Default Report Issue Form setup.
 */
export function buildInitialIssueFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return ISSUE_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const fields = ISSUE_FORM_FIELD_CATALOG.filter(
      (field) => field.sectionId === section.id,
    );
    const sectionColumns = clampIssueFormColumns(section.columns ?? 2);
...
export function cloneIssueFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneIssueFormTree(node.children || []),
  }));
}

export function isIssueFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isIssueFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenIssueFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenIssueFormTree(node.children, acc);
  });
  return acc;
}

export function findIssueFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findIssueFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findIssueFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findIssueFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getIssueFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findIssueFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findIssueFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getIssueFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findIssueFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findIssueFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getIssueFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getIssueFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveIssueFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isIssueFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveIssueFormLeaves(child));
}

export function collectAllActiveIssueFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveIssueFormLeaves(node));
}
...
export function filterIssueFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getIssueFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenIssueFormTree(nodes)
    .filter((node) => {
      if (isIssueFormValueField(node)) return false;
...
export function addIssueFormNode(nodes = [], payload) {
  const nextTree = cloneIssueFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findIssueFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isIssueFormValueField(parent)) {
  ...
  if (findIssueFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getIssueFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampIssueFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampIssueFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampIssueFormColSpan(
...
export function updateIssueFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneIssueFormTree(nodes);
  const existing = findIssueFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (name/description/active stay fixed).
  if (existing.isLocked) {
    const columns = clampIssueFormColumns(payload.columns ?? existing.columns ?? 2);
  ...
  if (existing.isDefaultLocked) {
    const l1 = getIssueFormLevel1Ancestor(nextTree, nodeId);
    const sectionColumns = clampIssueFormColumns(l1?.columns ?? 2);
  ...
  const isLeaf = isIssueFormLeaf(existing);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampIssueFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampIssueFormColSpan(
...
export function toggleIssueFormActive(nodes = [], nodeId) {
  const nextTree = cloneIssueFormTree(nodes);
  const existing = findIssueFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleIssueFormHidden(nodes = [], nodeId) {
  const nextTree = cloneIssueFormTree(nodes);
  const existing = findIssueFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteIssueFormNode(nodes = [], nodeId) {
  const existing = findIssueFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneIssueFormTree(nodes));
}

export function reorderIssueFormNode(nodes = [], nodeId, direction) {
  const { nodes: next, moved } = reorderTreeSibling(
    cloneIssueFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderIssueFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
...
export function getIssueFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the legacy field shape used by IssueFormWizard. */
export function leafToIssueField(leaf, sectionId) {
