/**
 * Issue Item Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  ISSUE_ITEM_FORM_GROUPS,
  ISSUE_ITEM_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./issueItemFormFields";
import {
...
export const ISSUE_ITEM_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const ISSUE_ITEM_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampIssueItemFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampIssueItemFormColSpan(value, columns = 2) {
  const max = clampIssueItemFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampIssueItemFormColumns(columns);
  // Issue Item defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampIssueItemFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const ISSUE_ITEM_FORM_LEVEL_1 = ISSUE_ITEM_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampIssueItemFormColSpan(
 ...
 */
export function buildInitialIssueItemFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return ISSUE_ITEM_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampIssueItemFormColumns(section.columns ?? 2);
    const groups = ISSUE_ITEM_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = ISSUE_ITEM_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampIssueItemFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = ISSUE_ITEM_FORM_FIELD_CATALOG.filter(
...
export function cloneIssueItemFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneIssueItemFormTree(node.children || []),
  }));
}

export function isIssueItemFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isIssueItemFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenIssueItemFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenIssueItemFormTree(node.children, acc);
  });
  return acc;
}

export function findIssueItemFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findIssueItemFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findIssueItemFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findIssueItemFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getIssueItemFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findIssueItemFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findIssueItemFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getIssueItemFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findIssueItemFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findIssueItemFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getIssueItemFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getIssueItemFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveIssueItemFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isIssueItemFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveIssueItemFormLeaves(child));
}

export function collectAllActiveIssueItemFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveIssueItemFormLeaves(node));
}
...
export function filterIssueItemFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getIssueItemFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenIssueItemFormTree(nodes)
    .filter((node) => {
      if (isIssueItemFormValueField(node)) return false;
...
export function addIssueItemFormNode(nodes = [], payload) {
  const nextTree = cloneIssueItemFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findIssueItemFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isIssueItemFormValueField(parent)) {
  ...
  if (findIssueItemFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getIssueItemFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampIssueItemFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampIssueItemFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampIssueItemFormColSpan(
...
export function updateIssueItemFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneIssueItemFormTree(nodes);
  const existing = findIssueItemFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampIssueItemFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampIssueItemFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findIssueItemFormParentNode(nextTree, nodeId);
    const sectionColumns = clampIssueItemFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampIssueItemFormColSpan(
  ...
  const isLeaf = isIssueItemFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getIssueItemFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampIssueItemFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampIssueItemFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampIssueItemFormColSpan(
...
export function toggleIssueItemFormActive(nodes = [], nodeId) {
  const nextTree = cloneIssueItemFormTree(nodes);
  const existing = findIssueItemFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleIssueItemFormHidden(nodes = [], nodeId) {
  const nextTree = cloneIssueItemFormTree(nodes);
  const existing = findIssueItemFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteIssueItemFormNode(nodes = [], nodeId) {
  const existing = findIssueItemFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneIssueItemFormTree(nodes));
}

export function reorderIssueItemFormNode(nodes = [], nodeId, direction) {
  const existing = findIssueItemFormNodeById(nodes, nodeId);
...
export function reorderIssueItemFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findIssueItemFormNodeById(nodes, sourceId);
...
export function getIssueItemFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Issue Item. */
export function leafToIssueItemField(leaf, sectionId) {
