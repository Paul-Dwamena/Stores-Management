/**
 * Leave Request Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  LEAVE_REQUEST_FORM_GROUPS,
  LEAVE_REQUEST_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./leaveRequestFormFields";
import {
...
export const LEAVE_REQUEST_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const LEAVE_REQUEST_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampLeaveRequestFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampLeaveRequestFormColSpan(value, columns = 2) {
  const max = clampLeaveRequestFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampLeaveRequestFormColumns(columns);
  // Leave Request defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampLeaveRequestFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const LEAVE_REQUEST_FORM_LEVEL_1 = LEAVE_REQUEST_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampLeaveRequestFormColSpan(
 ...
 */
export function buildInitialLeaveRequestFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return LEAVE_REQUEST_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampLeaveRequestFormColumns(section.columns ?? 2);
    const groups = LEAVE_REQUEST_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = LEAVE_REQUEST_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampLeaveRequestFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = LEAVE_REQUEST_FORM_FIELD_CATALOG.filter(
...
export function cloneLeaveRequestFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneLeaveRequestFormTree(node.children || []),
  }));
}

export function isLeaveRequestFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isLeaveRequestFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenLeaveRequestFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenLeaveRequestFormTree(node.children, acc);
  });
  return acc;
}

export function findLeaveRequestFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findLeaveRequestFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findLeaveRequestFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findLeaveRequestFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getLeaveRequestFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findLeaveRequestFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findLeaveRequestFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getLeaveRequestFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findLeaveRequestFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findLeaveRequestFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getLeaveRequestFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getLeaveRequestFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveLeaveRequestFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isLeaveRequestFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveLeaveRequestFormLeaves(child));
}

export function collectAllActiveLeaveRequestFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveLeaveRequestFormLeaves(node));
}
...
export function filterLeaveRequestFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getLeaveRequestFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenLeaveRequestFormTree(nodes)
    .filter((node) => {
      if (isLeaveRequestFormValueField(node)) return false;
...
export function addLeaveRequestFormNode(nodes = [], payload) {
  const nextTree = cloneLeaveRequestFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findLeaveRequestFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isLeaveRequestFormValueField(parent)) {
  ...
  if (findLeaveRequestFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getLeaveRequestFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampLeaveRequestFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampLeaveRequestFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampLeaveRequestFormColSpan(
...
export function updateLeaveRequestFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneLeaveRequestFormTree(nodes);
  const existing = findLeaveRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampLeaveRequestFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampLeaveRequestFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findLeaveRequestFormParentNode(nextTree, nodeId);
    const sectionColumns = clampLeaveRequestFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampLeaveRequestFormColSpan(
  ...
  const isLeaf = isLeaveRequestFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getLeaveRequestFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampLeaveRequestFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampLeaveRequestFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampLeaveRequestFormColSpan(
...
export function toggleLeaveRequestFormActive(nodes = [], nodeId) {
  const nextTree = cloneLeaveRequestFormTree(nodes);
  const existing = findLeaveRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleLeaveRequestFormHidden(nodes = [], nodeId) {
  const nextTree = cloneLeaveRequestFormTree(nodes);
  const existing = findLeaveRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteLeaveRequestFormNode(nodes = [], nodeId) {
  const existing = findLeaveRequestFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneLeaveRequestFormTree(nodes));
}

export function reorderLeaveRequestFormNode(nodes = [], nodeId, direction) {
  const existing = findLeaveRequestFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneLeaveRequestFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderLeaveRequestFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findLeaveRequestFormNodeById(nodes, sourceId);
...
export function getLeaveRequestFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Leave Request. */
export function leafToLeaveRequestField(leaf, sectionId) {
