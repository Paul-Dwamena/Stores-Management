/**
 * Legal document form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  LEGAL_DOCUMENT_FORM_FIELD_CATALOG,
  LEGAL_DOCUMENT_FORM_GROUPS,
  LEGAL_DOCUMENT_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./legalDocumentFormFields";
import {
...
export const LEGAL_DOCUMENT_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const LEGAL_DOCUMENT_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampLegalDocumentFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampLegalDocumentFormColSpan(value, columns = 2) {
  const max = clampLegalDocumentFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampLegalDocumentFormColumns(columns);
  // Upload Legal Documents defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampLegalDocumentFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const LEGAL_DOCUMENT_FORM_LEVEL_1 = LEGAL_DOCUMENT_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampLegalDocumentFormColSpan(
...
/**
 * Seed hierarchy: Main holds document file, name, issued by, and expiry fields.
 */
export function buildInitialLegalDocumentFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return LEGAL_DOCUMENT_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampLegalDocumentFormColumns(section.columns ?? 2);
    const groups = LEGAL_DOCUMENT_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = LEGAL_DOCUMENT_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampLegalDocumentFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = LEGAL_DOCUMENT_FORM_FIELD_CATALOG.filter(
...
export function cloneLegalDocumentFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneLegalDocumentFormTree(node.children || []),
  }));
}

export function isLegalDocumentFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isLegalDocumentFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenLegalDocumentFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenLegalDocumentFormTree(node.children, acc);
  });
  return acc;
}

export function findLegalDocumentFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findLegalDocumentFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findLegalDocumentFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findLegalDocumentFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getLegalDocumentFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findLegalDocumentFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findLegalDocumentFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getLegalDocumentFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findLegalDocumentFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findLegalDocumentFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getLegalDocumentFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getLegalDocumentFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveLegalDocumentFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isLegalDocumentFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveLegalDocumentFormLeaves(child));
}

export function collectAllActiveLegalDocumentFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveLegalDocumentFormLeaves(node));
}
...
export function filterLegalDocumentFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getLegalDocumentFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenLegalDocumentFormTree(nodes)
    .filter((node) => {
      if (isLegalDocumentFormValueField(node)) return false;
...
export function addLegalDocumentFormNode(nodes = [], payload) {
  const nextTree = cloneLegalDocumentFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findLegalDocumentFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isLegalDocumentFormValueField(parent)) {
  ...
  if (findLegalDocumentFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getLegalDocumentFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampLegalDocumentFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampLegalDocumentFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampLegalDocumentFormColSpan(
...
export function updateLegalDocumentFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneLegalDocumentFormTree(nodes);
  const existing = findLegalDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampLegalDocumentFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampLegalDocumentFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findLegalDocumentFormParentNode(nextTree, nodeId);
    const sectionColumns = clampLegalDocumentFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampLegalDocumentFormColSpan(
  ...
  const isLeaf = isLegalDocumentFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getLegalDocumentFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampLegalDocumentFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampLegalDocumentFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampLegalDocumentFormColSpan(
...
export function toggleLegalDocumentFormActive(nodes = [], nodeId) {
  const nextTree = cloneLegalDocumentFormTree(nodes);
  const existing = findLegalDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleLegalDocumentFormHidden(nodes = [], nodeId) {
  const nextTree = cloneLegalDocumentFormTree(nodes);
  const existing = findLegalDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteLegalDocumentFormNode(nodes = [], nodeId) {
  const existing = findLegalDocumentFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneLegalDocumentFormTree(nodes));
}

export function reorderLegalDocumentFormNode(nodes = [], nodeId, direction) {
  const existing = findLegalDocumentFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneLegalDocumentFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderLegalDocumentFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findLegalDocumentFormNodeById(nodes, sourceId);
...
export function getLegalDocumentFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Upload Legal Documents. */
export function leafToLegalDocumentField(leaf, sectionId) {
