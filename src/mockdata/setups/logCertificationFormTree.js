/**
 * Log certification form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  LOG_CERTIFICATION_FORM_FIELD_CATALOG,
  LOG_CERTIFICATION_FORM_GROUPS,
  LOG_CERTIFICATION_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./logCertificationFormFields";
import {
...
export const LOG_CERTIFICATION_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const LOG_CERTIFICATION_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampLogCertificationFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampLogCertificationFormColSpan(value, columns = 2) {
  const max = clampLogCertificationFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampLogCertificationFormColumns(columns);
  // Log Certification defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampLogCertificationFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const LOG_CERTIFICATION_FORM_LEVEL_1 = LOG_CERTIFICATION_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampLogCertificationFormColSpan(
...
/**
 * Seed hierarchy: Main holds driver, certification name, dates, and assessor fields.
 */
export function buildInitialLogCertificationFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return LOG_CERTIFICATION_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampLogCertificationFormColumns(section.columns ?? 2);
    const groups = LOG_CERTIFICATION_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = LOG_CERTIFICATION_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampLogCertificationFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = LOG_CERTIFICATION_FORM_FIELD_CATALOG.filter(
...
export function cloneLogCertificationFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneLogCertificationFormTree(node.children || []),
  }));
}

export function isLogCertificationFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isLogCertificationFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenLogCertificationFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenLogCertificationFormTree(node.children, acc);
  });
  return acc;
}

export function findLogCertificationFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findLogCertificationFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findLogCertificationFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findLogCertificationFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getLogCertificationFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findLogCertificationFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findLogCertificationFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getLogCertificationFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findLogCertificationFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findLogCertificationFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getLogCertificationFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getLogCertificationFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveLogCertificationFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isLogCertificationFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveLogCertificationFormLeaves(child));
}

export function collectAllActiveLogCertificationFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveLogCertificationFormLeaves(node));
}
...
export function filterLogCertificationFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getLogCertificationFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenLogCertificationFormTree(nodes)
    .filter((node) => {
      if (isLogCertificationFormValueField(node)) return false;
...
export function addLogCertificationFormNode(nodes = [], payload) {
  const nextTree = cloneLogCertificationFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findLogCertificationFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isLogCertificationFormValueField(parent)) {
  ...
  if (findLogCertificationFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getLogCertificationFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampLogCertificationFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampLogCertificationFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampLogCertificationFormColSpan(
...
export function updateLogCertificationFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneLogCertificationFormTree(nodes);
  const existing = findLogCertificationFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampLogCertificationFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampLogCertificationFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findLogCertificationFormParentNode(nextTree, nodeId);
    const sectionColumns = clampLogCertificationFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampLogCertificationFormColSpan(
  ...
  const isLeaf = isLogCertificationFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getLogCertificationFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampLogCertificationFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampLogCertificationFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampLogCertificationFormColSpan(
...
export function toggleLogCertificationFormActive(nodes = [], nodeId) {
  const nextTree = cloneLogCertificationFormTree(nodes);
  const existing = findLogCertificationFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleLogCertificationFormHidden(nodes = [], nodeId) {
  const nextTree = cloneLogCertificationFormTree(nodes);
  const existing = findLogCertificationFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteLogCertificationFormNode(nodes = [], nodeId) {
  const existing = findLogCertificationFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneLogCertificationFormTree(nodes));
}

export function reorderLogCertificationFormNode(nodes = [], nodeId, direction) {
  const existing = findLogCertificationFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneLogCertificationFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderLogCertificationFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findLogCertificationFormNodeById(nodes, sourceId);
...
export function getLogCertificationFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Log Certification. */
export function leafToLogCertificationField(leaf, sectionId) {
