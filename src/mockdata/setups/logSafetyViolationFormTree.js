/**
 * Log safety violation form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  LOG_SAFETY_VIOLATION_FORM_FIELD_CATALOG,
  LOG_SAFETY_VIOLATION_FORM_GROUPS,
  LOG_SAFETY_VIOLATION_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./logSafetyViolationFormFields";
import {
...
export const LOG_SAFETY_VIOLATION_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const LOG_SAFETY_VIOLATION_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampLogSafetyViolationFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampLogSafetyViolationFormColSpan(value, columns = 2) {
  const max = clampLogSafetyViolationFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampLogSafetyViolationFormColumns(columns);
  // Log Safety Violation defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampLogSafetyViolationFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const LOG_SAFETY_VIOLATION_FORM_LEVEL_1 = LOG_SAFETY_VIOLATION_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampLogSafetyViolationFormColSpan(
...
/**
 * Seed hierarchy: Main holds driver, vehicle, infraction type, severity, penalty, and notes fields.
 */
export function buildInitialLogSafetyViolationFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return LOG_SAFETY_VIOLATION_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampLogSafetyViolationFormColumns(section.columns ?? 2);
    const groups = LOG_SAFETY_VIOLATION_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = LOG_SAFETY_VIOLATION_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampLogSafetyViolationFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = LOG_SAFETY_VIOLATION_FORM_FIELD_CATALOG.filter(
...
export function cloneLogSafetyViolationFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneLogSafetyViolationFormTree(node.children || []),
  }));
}

export function isLogSafetyViolationFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isLogSafetyViolationFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenLogSafetyViolationFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenLogSafetyViolationFormTree(node.children, acc);
  });
  return acc;
}

export function findLogSafetyViolationFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findLogSafetyViolationFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findLogSafetyViolationFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findLogSafetyViolationFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getLogSafetyViolationFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findLogSafetyViolationFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findLogSafetyViolationFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getLogSafetyViolationFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findLogSafetyViolationFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findLogSafetyViolationFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getLogSafetyViolationFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getLogSafetyViolationFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveLogSafetyViolationFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isLogSafetyViolationFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveLogSafetyViolationFormLeaves(child));
}

export function collectAllActiveLogSafetyViolationFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveLogSafetyViolationFormLeaves(node));
}
...
export function filterLogSafetyViolationFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getLogSafetyViolationFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenLogSafetyViolationFormTree(nodes)
    .filter((node) => {
      if (isLogSafetyViolationFormValueField(node)) return false;
...
export function addLogSafetyViolationFormNode(nodes = [], payload) {
  const nextTree = cloneLogSafetyViolationFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findLogSafetyViolationFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isLogSafetyViolationFormValueField(parent)) {
  ...
  if (findLogSafetyViolationFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getLogSafetyViolationFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampLogSafetyViolationFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampLogSafetyViolationFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampLogSafetyViolationFormColSpan(
...
export function updateLogSafetyViolationFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneLogSafetyViolationFormTree(nodes);
  const existing = findLogSafetyViolationFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampLogSafetyViolationFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampLogSafetyViolationFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findLogSafetyViolationFormParentNode(nextTree, nodeId);
    const sectionColumns = clampLogSafetyViolationFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampLogSafetyViolationFormColSpan(
  ...
  const isLeaf = isLogSafetyViolationFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getLogSafetyViolationFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampLogSafetyViolationFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampLogSafetyViolationFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampLogSafetyViolationFormColSpan(
...
export function toggleLogSafetyViolationFormActive(nodes = [], nodeId) {
  const nextTree = cloneLogSafetyViolationFormTree(nodes);
  const existing = findLogSafetyViolationFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleLogSafetyViolationFormHidden(nodes = [], nodeId) {
  const nextTree = cloneLogSafetyViolationFormTree(nodes);
  const existing = findLogSafetyViolationFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteLogSafetyViolationFormNode(nodes = [], nodeId) {
  const existing = findLogSafetyViolationFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneLogSafetyViolationFormTree(nodes));
}

export function reorderLogSafetyViolationFormNode(nodes = [], nodeId, direction) {
  const existing = findLogSafetyViolationFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneLogSafetyViolationFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderLogSafetyViolationFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findLogSafetyViolationFormNodeById(nodes, sourceId);
...
export function getLogSafetyViolationFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Log Safety Violation. */
export function leafToLogSafetyViolationField(leaf, sectionId) {
