/**
 * Maintenance schedule form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  MAINTENANCE_SCHEDULE_FORM_FIELD_CATALOG,
  MAINTENANCE_SCHEDULE_FORM_GROUPS,
  MAINTENANCE_SCHEDULE_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./maintenanceScheduleFormFields";
import {
...
export const MAINTENANCE_SCHEDULE_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const MAINTENANCE_SCHEDULE_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampMaintenanceScheduleFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampMaintenanceScheduleFormColSpan(value, columns = 2) {
  const max = clampMaintenanceScheduleFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampMaintenanceScheduleFormColumns(columns);
  // Create Maintenance Schedule defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampMaintenanceScheduleFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const MAINTENANCE_SCHEDULE_FORM_LEVEL_1 = MAINTENANCE_SCHEDULE_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampMaintenanceScheduleFormColSpan(
...
/**
 * Seed hierarchy: Main holds vehicle, type, trigger, and priority fields.
 */
export function buildInitialMaintenanceScheduleFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return MAINTENANCE_SCHEDULE_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampMaintenanceScheduleFormColumns(section.columns ?? 2);
    const groups = MAINTENANCE_SCHEDULE_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = MAINTENANCE_SCHEDULE_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampMaintenanceScheduleFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = MAINTENANCE_SCHEDULE_FORM_FIELD_CATALOG.filter(
...
export function cloneMaintenanceScheduleFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneMaintenanceScheduleFormTree(node.children || []),
  }));
}

export function isMaintenanceScheduleFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isMaintenanceScheduleFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenMaintenanceScheduleFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenMaintenanceScheduleFormTree(node.children, acc);
  });
  return acc;
}

export function findMaintenanceScheduleFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findMaintenanceScheduleFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findMaintenanceScheduleFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findMaintenanceScheduleFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getMaintenanceScheduleFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findMaintenanceScheduleFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findMaintenanceScheduleFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getMaintenanceScheduleFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findMaintenanceScheduleFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findMaintenanceScheduleFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getMaintenanceScheduleFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getMaintenanceScheduleFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveMaintenanceScheduleFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isMaintenanceScheduleFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveMaintenanceScheduleFormLeaves(child));
}

export function collectAllActiveMaintenanceScheduleFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveMaintenanceScheduleFormLeaves(node));
}
...
export function filterMaintenanceScheduleFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getMaintenanceScheduleFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenMaintenanceScheduleFormTree(nodes)
    .filter((node) => {
      if (isMaintenanceScheduleFormValueField(node)) return false;
...
export function addMaintenanceScheduleFormNode(nodes = [], payload) {
  const nextTree = cloneMaintenanceScheduleFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findMaintenanceScheduleFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isMaintenanceScheduleFormValueField(parent)) {
  ...
  if (findMaintenanceScheduleFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getMaintenanceScheduleFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampMaintenanceScheduleFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampMaintenanceScheduleFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampMaintenanceScheduleFormColSpan(
...
export function updateMaintenanceScheduleFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneMaintenanceScheduleFormTree(nodes);
  const existing = findMaintenanceScheduleFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampMaintenanceScheduleFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampMaintenanceScheduleFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findMaintenanceScheduleFormParentNode(nextTree, nodeId);
    const sectionColumns = clampMaintenanceScheduleFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampMaintenanceScheduleFormColSpan(
  ...
  const isLeaf = isMaintenanceScheduleFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getMaintenanceScheduleFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampMaintenanceScheduleFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampMaintenanceScheduleFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampMaintenanceScheduleFormColSpan(
...
export function toggleMaintenanceScheduleFormActive(nodes = [], nodeId) {
  const nextTree = cloneMaintenanceScheduleFormTree(nodes);
  const existing = findMaintenanceScheduleFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleMaintenanceScheduleFormHidden(nodes = [], nodeId) {
  const nextTree = cloneMaintenanceScheduleFormTree(nodes);
  const existing = findMaintenanceScheduleFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteMaintenanceScheduleFormNode(nodes = [], nodeId) {
  const existing = findMaintenanceScheduleFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneMaintenanceScheduleFormTree(nodes));
}

export function reorderMaintenanceScheduleFormNode(nodes = [], nodeId, direction) {
  const existing = findMaintenanceScheduleFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneMaintenanceScheduleFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderMaintenanceScheduleFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findMaintenanceScheduleFormNodeById(nodes, sourceId);
...
export function getMaintenanceScheduleFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Create Maintenance Schedule. */
export function leafToMaintenanceScheduleField(leaf, sectionId) {
