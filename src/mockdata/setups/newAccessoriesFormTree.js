/**
 * New Accessories (Unregistered) Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  NEW_ACCESSORIES_FORM_GROUPS,
  NEW_ACCESSORIES_FORM_SECTIONS,
...
export const NEW_ACCESSORIES_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const NEW_ACCESSORIES_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampNewAccessoriesFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampNewAccessoriesFormColSpan(value, columns = 2) {
  const max = clampNewAccessoriesFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampNewAccessoriesFormColumns(columns);
  // New Accessories (Unregistered) defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampNewAccessoriesFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const NEW_ACCESSORIES_FORM_LEVEL_1 = NEW_ACCESSORIES_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampNewAccessoriesFormColSpan(
 ...
 */
export function buildInitialNewAccessoriesFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return NEW_ACCESSORIES_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampNewAccessoriesFormColumns(section.columns ?? 2);
    const groups = NEW_ACCESSORIES_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = NEW_ACCESSORIES_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampNewAccessoriesFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = NEW_ACCESSORIES_FORM_FIELD_CATALOG.filter(
...
export function cloneNewAccessoriesFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneNewAccessoriesFormTree(node.children || []),
  }));
}

export function isNewAccessoriesFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isNewAccessoriesFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenNewAccessoriesFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenNewAccessoriesFormTree(node.children, acc);
  });
  return acc;
}

export function findNewAccessoriesFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findNewAccessoriesFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findNewAccessoriesFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findNewAccessoriesFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getNewAccessoriesFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findNewAccessoriesFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findNewAccessoriesFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getNewAccessoriesFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findNewAccessoriesFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findNewAccessoriesFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getNewAccessoriesFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getNewAccessoriesFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveNewAccessoriesFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isNewAccessoriesFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveNewAccessoriesFormLeaves(child));
}

export function collectAllActiveNewAccessoriesFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveNewAccessoriesFormLeaves(node));
}
...
export function filterNewAccessoriesFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getNewAccessoriesFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenNewAccessoriesFormTree(nodes)
    .filter((node) => {
      if (isNewAccessoriesFormValueField(node)) return false;
...
export function addNewAccessoriesFormNode(nodes = [], payload) {
  const nextTree = cloneNewAccessoriesFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findNewAccessoriesFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isNewAccessoriesFormValueField(parent)) {
  ...
  if (findNewAccessoriesFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getNewAccessoriesFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampNewAccessoriesFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampNewAccessoriesFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampNewAccessoriesFormColSpan(
...
export function updateNewAccessoriesFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneNewAccessoriesFormTree(nodes);
  const existing = findNewAccessoriesFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampNewAccessoriesFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampNewAccessoriesFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findNewAccessoriesFormParentNode(nextTree, nodeId);
    const sectionColumns = clampNewAccessoriesFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampNewAccessoriesFormColSpan(
  ...
  const isLeaf = isNewAccessoriesFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getNewAccessoriesFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampNewAccessoriesFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampNewAccessoriesFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampNewAccessoriesFormColSpan(
...
export function toggleNewAccessoriesFormActive(nodes = [], nodeId) {
  const nextTree = cloneNewAccessoriesFormTree(nodes);
  const existing = findNewAccessoriesFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleNewAccessoriesFormHidden(nodes = [], nodeId) {
  const nextTree = cloneNewAccessoriesFormTree(nodes);
  const existing = findNewAccessoriesFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteNewAccessoriesFormNode(nodes = [], nodeId) {
  const existing = findNewAccessoriesFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneNewAccessoriesFormTree(nodes));
}

export function reorderNewAccessoriesFormNode(nodes = [], nodeId, direction) {
  const existing = findNewAccessoriesFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneNewAccessoriesFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderNewAccessoriesFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findNewAccessoriesFormNodeById(nodes, sourceId);
...
export function getNewAccessoriesFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by New Accessories (Unregistered). */
export function leafToNewAccessoriesField(leaf, sectionId) {
