/**
 * New Funding Request Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  NEW_FUNDING_REQUEST_FORM_FIELD_CATALOG,
  NEW_FUNDING_REQUEST_FORM_GROUPS,
  NEW_FUNDING_REQUEST_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./newFundingRequestFormFields";
import {
...
export const NEW_FUNDING_REQUEST_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const NEW_FUNDING_REQUEST_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampNewFundingRequestFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampNewFundingRequestFormColSpan(value, columns = 2) {
  const max = clampNewFundingRequestFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampNewFundingRequestFormColumns(columns);
  // New Funding Request defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampNewFundingRequestFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const NEW_FUNDING_REQUEST_FORM_LEVEL_1 = NEW_FUNDING_REQUEST_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampNewFundingRequestFormColSpan(
 ...
 */
export function buildInitialNewFundingRequestFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return NEW_FUNDING_REQUEST_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampNewFundingRequestFormColumns(section.columns ?? 2);
    const groups = NEW_FUNDING_REQUEST_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = NEW_FUNDING_REQUEST_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampNewFundingRequestFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = NEW_FUNDING_REQUEST_FORM_FIELD_CATALOG.filter(
...
export function cloneNewFundingRequestFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneNewFundingRequestFormTree(node.children || []),
  }));
}

export function isNewFundingRequestFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isNewFundingRequestFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenNewFundingRequestFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenNewFundingRequestFormTree(node.children, acc);
  });
  return acc;
}

export function findNewFundingRequestFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findNewFundingRequestFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findNewFundingRequestFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findNewFundingRequestFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getNewFundingRequestFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findNewFundingRequestFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findNewFundingRequestFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getNewFundingRequestFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findNewFundingRequestFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findNewFundingRequestFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getNewFundingRequestFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getNewFundingRequestFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveNewFundingRequestFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isNewFundingRequestFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveNewFundingRequestFormLeaves(child));
}

export function collectAllActiveNewFundingRequestFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveNewFundingRequestFormLeaves(node));
}
...
export function filterNewFundingRequestFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getNewFundingRequestFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenNewFundingRequestFormTree(nodes)
    .filter((node) => {
      if (isNewFundingRequestFormValueField(node)) return false;
...
export function addNewFundingRequestFormNode(nodes = [], payload) {
  const nextTree = cloneNewFundingRequestFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findNewFundingRequestFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isNewFundingRequestFormValueField(parent)) {
  ...
  if (findNewFundingRequestFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getNewFundingRequestFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampNewFundingRequestFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampNewFundingRequestFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampNewFundingRequestFormColSpan(
...
export function updateNewFundingRequestFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneNewFundingRequestFormTree(nodes);
  const existing = findNewFundingRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampNewFundingRequestFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampNewFundingRequestFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findNewFundingRequestFormParentNode(nextTree, nodeId);
    const sectionColumns = clampNewFundingRequestFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampNewFundingRequestFormColSpan(
  ...
  const isLeaf = isNewFundingRequestFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getNewFundingRequestFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampNewFundingRequestFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampNewFundingRequestFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampNewFundingRequestFormColSpan(
...
export function toggleNewFundingRequestFormActive(nodes = [], nodeId) {
  const nextTree = cloneNewFundingRequestFormTree(nodes);
  const existing = findNewFundingRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleNewFundingRequestFormHidden(nodes = [], nodeId) {
  const nextTree = cloneNewFundingRequestFormTree(nodes);
  const existing = findNewFundingRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteNewFundingRequestFormNode(nodes = [], nodeId) {
  const existing = findNewFundingRequestFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneNewFundingRequestFormTree(nodes));
}

export function reorderNewFundingRequestFormNode(nodes = [], nodeId, direction) {
  const existing = findNewFundingRequestFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneNewFundingRequestFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderNewFundingRequestFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findNewFundingRequestFormNodeById(nodes, sourceId);
...
export function getNewFundingRequestFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by New Funding Request. */
export function leafToNewFundingRequestField(leaf, sectionId) {
