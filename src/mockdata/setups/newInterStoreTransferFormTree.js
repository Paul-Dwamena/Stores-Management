/**
 * New Inter-store Transfer Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  NEW_INTER_STORE_TRANSFER_FORM_GROUPS,
  NEW_INTER_STORE_TRANSFER_FORM_SECTIONS,
...
export const NEW_INTER_STORE_TRANSFER_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const NEW_INTER_STORE_TRANSFER_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampNewInterStoreTransferFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampNewInterStoreTransferFormColSpan(value, columns = 2) {
  const max = clampNewInterStoreTransferFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampNewInterStoreTransferFormColumns(columns);
  // New Inter-store Transfer defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampNewInterStoreTransferFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const NEW_INTER_STORE_TRANSFER_FORM_LEVEL_1 = NEW_INTER_STORE_TRANSFER_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampNewInterStoreTransferFormColSpan(
 ...
 */
export function buildInitialNewInterStoreTransferFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return NEW_INTER_STORE_TRANSFER_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampNewInterStoreTransferFormColumns(section.columns ?? 2);
    const groups = NEW_INTER_STORE_TRANSFER_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = NEW_INTER_STORE_TRANSFER_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampNewInterStoreTransferFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = NEW_INTER_STORE_TRANSFER_FORM_FIELD_CATALOG.filter(
...
export function cloneNewInterStoreTransferFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneNewInterStoreTransferFormTree(node.children || []),
  }));
}

export function isNewInterStoreTransferFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isNewInterStoreTransferFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenNewInterStoreTransferFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenNewInterStoreTransferFormTree(node.children, acc);
  });
  return acc;
}

export function findNewInterStoreTransferFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findNewInterStoreTransferFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findNewInterStoreTransferFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findNewInterStoreTransferFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getNewInterStoreTransferFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findNewInterStoreTransferFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findNewInterStoreTransferFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getNewInterStoreTransferFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findNewInterStoreTransferFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findNewInterStoreTransferFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getNewInterStoreTransferFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getNewInterStoreTransferFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveNewInterStoreTransferFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isNewInterStoreTransferFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveNewInterStoreTransferFormLeaves(child));
}

export function collectAllActiveNewInterStoreTransferFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveNewInterStoreTransferFormLeaves(node));
}
...
export function filterNewInterStoreTransferFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getNewInterStoreTransferFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenNewInterStoreTransferFormTree(nodes)
    .filter((node) => {
      if (isNewInterStoreTransferFormValueField(node)) return false;
...
export function addNewInterStoreTransferFormNode(nodes = [], payload) {
  const nextTree = cloneNewInterStoreTransferFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findNewInterStoreTransferFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isNewInterStoreTransferFormValueField(parent)) {
  ...
  if (findNewInterStoreTransferFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getNewInterStoreTransferFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampNewInterStoreTransferFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampNewInterStoreTransferFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampNewInterStoreTransferFormColSpan(
...
export function updateNewInterStoreTransferFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneNewInterStoreTransferFormTree(nodes);
  const existing = findNewInterStoreTransferFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampNewInterStoreTransferFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampNewInterStoreTransferFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findNewInterStoreTransferFormParentNode(nextTree, nodeId);
    const sectionColumns = clampNewInterStoreTransferFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampNewInterStoreTransferFormColSpan(
  ...
  const isLeaf = isNewInterStoreTransferFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getNewInterStoreTransferFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampNewInterStoreTransferFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampNewInterStoreTransferFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampNewInterStoreTransferFormColSpan(
...
export function toggleNewInterStoreTransferFormActive(nodes = [], nodeId) {
  const nextTree = cloneNewInterStoreTransferFormTree(nodes);
  const existing = findNewInterStoreTransferFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleNewInterStoreTransferFormHidden(nodes = [], nodeId) {
  const nextTree = cloneNewInterStoreTransferFormTree(nodes);
  const existing = findNewInterStoreTransferFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteNewInterStoreTransferFormNode(nodes = [], nodeId) {
  const existing = findNewInterStoreTransferFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneNewInterStoreTransferFormTree(nodes));
}

export function reorderNewInterStoreTransferFormNode(nodes = [], nodeId, direction) {
  const existing = findNewInterStoreTransferFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneNewInterStoreTransferFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderNewInterStoreTransferFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findNewInterStoreTransferFormNodeById(nodes, sourceId);
...
export function getNewInterStoreTransferFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by New Inter-store Transfer. */
export function leafToNewInterStoreTransferField(leaf, sectionId) {
