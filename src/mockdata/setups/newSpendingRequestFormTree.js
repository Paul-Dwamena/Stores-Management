/**
 * New Spending Request Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  NEW_SPENDING_REQUEST_FORM_FIELD_CATALOG,
  NEW_SPENDING_REQUEST_FORM_GROUPS,
  NEW_SPENDING_REQUEST_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./newSpendingRequestFormFields";
import {
...
export const NEW_SPENDING_REQUEST_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const NEW_SPENDING_REQUEST_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampNewSpendingRequestFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampNewSpendingRequestFormColSpan(value, columns = 2) {
  const max = clampNewSpendingRequestFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampNewSpendingRequestFormColumns(columns);
  // New Spending Request defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampNewSpendingRequestFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const NEW_SPENDING_REQUEST_FORM_LEVEL_1 = NEW_SPENDING_REQUEST_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampNewSpendingRequestFormColSpan(
 ...
 */
export function buildInitialNewSpendingRequestFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return NEW_SPENDING_REQUEST_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampNewSpendingRequestFormColumns(section.columns ?? 2);
    const groups = NEW_SPENDING_REQUEST_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = NEW_SPENDING_REQUEST_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampNewSpendingRequestFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = NEW_SPENDING_REQUEST_FORM_FIELD_CATALOG.filter(
...
export function cloneNewSpendingRequestFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneNewSpendingRequestFormTree(node.children || []),
  }));
}

export function isNewSpendingRequestFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isNewSpendingRequestFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenNewSpendingRequestFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenNewSpendingRequestFormTree(node.children, acc);
  });
  return acc;
}

export function findNewSpendingRequestFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findNewSpendingRequestFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findNewSpendingRequestFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findNewSpendingRequestFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getNewSpendingRequestFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findNewSpendingRequestFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findNewSpendingRequestFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getNewSpendingRequestFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findNewSpendingRequestFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findNewSpendingRequestFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getNewSpendingRequestFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getNewSpendingRequestFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveNewSpendingRequestFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isNewSpendingRequestFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveNewSpendingRequestFormLeaves(child));
}

export function collectAllActiveNewSpendingRequestFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveNewSpendingRequestFormLeaves(node));
}
...
export function filterNewSpendingRequestFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getNewSpendingRequestFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenNewSpendingRequestFormTree(nodes)
    .filter((node) => {
      if (isNewSpendingRequestFormValueField(node)) return false;
...
export function addNewSpendingRequestFormNode(nodes = [], payload) {
  const nextTree = cloneNewSpendingRequestFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findNewSpendingRequestFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isNewSpendingRequestFormValueField(parent)) {
  ...
  if (findNewSpendingRequestFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getNewSpendingRequestFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampNewSpendingRequestFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampNewSpendingRequestFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampNewSpendingRequestFormColSpan(
...
export function updateNewSpendingRequestFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneNewSpendingRequestFormTree(nodes);
  const existing = findNewSpendingRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampNewSpendingRequestFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampNewSpendingRequestFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findNewSpendingRequestFormParentNode(nextTree, nodeId);
    const sectionColumns = clampNewSpendingRequestFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampNewSpendingRequestFormColSpan(
  ...
  const isLeaf = isNewSpendingRequestFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getNewSpendingRequestFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampNewSpendingRequestFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampNewSpendingRequestFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampNewSpendingRequestFormColSpan(
...
export function toggleNewSpendingRequestFormActive(nodes = [], nodeId) {
  const nextTree = cloneNewSpendingRequestFormTree(nodes);
  const existing = findNewSpendingRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleNewSpendingRequestFormHidden(nodes = [], nodeId) {
  const nextTree = cloneNewSpendingRequestFormTree(nodes);
  const existing = findNewSpendingRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteNewSpendingRequestFormNode(nodes = [], nodeId) {
  const existing = findNewSpendingRequestFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneNewSpendingRequestFormTree(nodes));
}

export function reorderNewSpendingRequestFormNode(nodes = [], nodeId, direction) {
  const existing = findNewSpendingRequestFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneNewSpendingRequestFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderNewSpendingRequestFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findNewSpendingRequestFormNodeById(nodes, sourceId);
...
export function getNewSpendingRequestFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by New Spending Request. */
export function leafToNewSpendingRequestField(leaf, sectionId) {
