/**
 * Quick Add Vehicle Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  QUICK_ADD_VEHICLE_FORM_FIELD_CATALOG,
  QUICK_ADD_VEHICLE_FORM_GROUPS,
  QUICK_ADD_VEHICLE_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./quickAddVehicleFormFields";
import {
...
export const QUICK_ADD_VEHICLE_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const QUICK_ADD_VEHICLE_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampQuickAddVehicleFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampQuickAddVehicleFormColSpan(value, columns = 2) {
  const max = clampQuickAddVehicleFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampQuickAddVehicleFormColumns(columns);
  // Quick Add Vehicle defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampQuickAddVehicleFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const QUICK_ADD_VEHICLE_FORM_LEVEL_1 = QUICK_ADD_VEHICLE_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampQuickAddVehicleFormColSpan(
...
/**
 * Seed hierarchy: Main holds VIN, catalog make/model/body/trim, name, and plate.
 */
export function buildInitialQuickAddVehicleFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return QUICK_ADD_VEHICLE_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampQuickAddVehicleFormColumns(section.columns ?? 2);
    const groups = QUICK_ADD_VEHICLE_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = QUICK_ADD_VEHICLE_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampQuickAddVehicleFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = QUICK_ADD_VEHICLE_FORM_FIELD_CATALOG.filter(
...
export function cloneQuickAddVehicleFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneQuickAddVehicleFormTree(node.children || []),
  }));
}

export function isQuickAddVehicleFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isQuickAddVehicleFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenQuickAddVehicleFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenQuickAddVehicleFormTree(node.children, acc);
  });
  return acc;
}

export function findQuickAddVehicleFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findQuickAddVehicleFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findQuickAddVehicleFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findQuickAddVehicleFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getQuickAddVehicleFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findQuickAddVehicleFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findQuickAddVehicleFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getQuickAddVehicleFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findQuickAddVehicleFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findQuickAddVehicleFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getQuickAddVehicleFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getQuickAddVehicleFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveQuickAddVehicleFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isQuickAddVehicleFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveQuickAddVehicleFormLeaves(child));
}

export function collectAllActiveQuickAddVehicleFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveQuickAddVehicleFormLeaves(node));
}
...
export function filterQuickAddVehicleFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getQuickAddVehicleFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenQuickAddVehicleFormTree(nodes)
    .filter((node) => {
      if (isQuickAddVehicleFormValueField(node)) return false;
...
export function addQuickAddVehicleFormNode(nodes = [], payload) {
  const nextTree = cloneQuickAddVehicleFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findQuickAddVehicleFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isQuickAddVehicleFormValueField(parent)) {
  ...
  if (findQuickAddVehicleFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getQuickAddVehicleFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampQuickAddVehicleFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampQuickAddVehicleFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampQuickAddVehicleFormColSpan(
...
export function updateQuickAddVehicleFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneQuickAddVehicleFormTree(nodes);
  const existing = findQuickAddVehicleFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampQuickAddVehicleFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampQuickAddVehicleFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findQuickAddVehicleFormParentNode(nextTree, nodeId);
    const sectionColumns = clampQuickAddVehicleFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampQuickAddVehicleFormColSpan(
  ...
  const isLeaf = isQuickAddVehicleFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getQuickAddVehicleFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampQuickAddVehicleFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampQuickAddVehicleFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampQuickAddVehicleFormColSpan(
...
export function toggleQuickAddVehicleFormActive(nodes = [], nodeId) {
  const nextTree = cloneQuickAddVehicleFormTree(nodes);
  const existing = findQuickAddVehicleFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleQuickAddVehicleFormHidden(nodes = [], nodeId) {
  const nextTree = cloneQuickAddVehicleFormTree(nodes);
  const existing = findQuickAddVehicleFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteQuickAddVehicleFormNode(nodes = [], nodeId) {
  const existing = findQuickAddVehicleFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneQuickAddVehicleFormTree(nodes));
}

export function reorderQuickAddVehicleFormNode(nodes = [], nodeId, direction) {
  const existing = findQuickAddVehicleFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneQuickAddVehicleFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderQuickAddVehicleFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findQuickAddVehicleFormNodeById(nodes, sourceId);
...
export function getQuickAddVehicleFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Quick Add Vehicle. */
export function leafToQuickAddVehicleField(leaf, sectionId) {
