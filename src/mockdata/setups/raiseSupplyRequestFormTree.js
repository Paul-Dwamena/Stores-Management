/**
 * Raise Supply Request Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  RAISE_SUPPLY_REQUEST_FORM_GROUPS,
  RAISE_SUPPLY_REQUEST_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./raiseSupplyRequestFormFields";
import {
...
export const RAISE_SUPPLY_REQUEST_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const RAISE_SUPPLY_REQUEST_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampRaiseSupplyRequestFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampRaiseSupplyRequestFormColSpan(value, columns = 2) {
  const max = clampRaiseSupplyRequestFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampRaiseSupplyRequestFormColumns(columns);
  // Raise Supply Request defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampRaiseSupplyRequestFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const RAISE_SUPPLY_REQUEST_FORM_LEVEL_1 = RAISE_SUPPLY_REQUEST_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampRaiseSupplyRequestFormColSpan(
 ...
 */
export function buildInitialRaiseSupplyRequestFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return RAISE_SUPPLY_REQUEST_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampRaiseSupplyRequestFormColumns(section.columns ?? 2);
    const groups = RAISE_SUPPLY_REQUEST_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = RAISE_SUPPLY_REQUEST_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampRaiseSupplyRequestFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = RAISE_SUPPLY_REQUEST_FORM_FIELD_CATALOG.filter(
...
export function cloneRaiseSupplyRequestFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneRaiseSupplyRequestFormTree(node.children || []),
  }));
}

export function isRaiseSupplyRequestFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isRaiseSupplyRequestFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenRaiseSupplyRequestFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenRaiseSupplyRequestFormTree(node.children, acc);
  });
  return acc;
}

export function findRaiseSupplyRequestFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findRaiseSupplyRequestFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findRaiseSupplyRequestFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findRaiseSupplyRequestFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getRaiseSupplyRequestFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findRaiseSupplyRequestFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findRaiseSupplyRequestFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getRaiseSupplyRequestFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findRaiseSupplyRequestFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findRaiseSupplyRequestFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getRaiseSupplyRequestFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getRaiseSupplyRequestFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveRaiseSupplyRequestFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isRaiseSupplyRequestFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveRaiseSupplyRequestFormLeaves(child));
}

export function collectAllActiveRaiseSupplyRequestFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveRaiseSupplyRequestFormLeaves(node));
}
...
export function filterRaiseSupplyRequestFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getRaiseSupplyRequestFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenRaiseSupplyRequestFormTree(nodes)
    .filter((node) => {
      if (isRaiseSupplyRequestFormValueField(node)) return false;
...
export function addRaiseSupplyRequestFormNode(nodes = [], payload) {
  const nextTree = cloneRaiseSupplyRequestFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findRaiseSupplyRequestFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isRaiseSupplyRequestFormValueField(parent)) {
  ...
  if (findRaiseSupplyRequestFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getRaiseSupplyRequestFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampRaiseSupplyRequestFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampRaiseSupplyRequestFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampRaiseSupplyRequestFormColSpan(
...
export function updateRaiseSupplyRequestFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneRaiseSupplyRequestFormTree(nodes);
  const existing = findRaiseSupplyRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampRaiseSupplyRequestFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampRaiseSupplyRequestFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findRaiseSupplyRequestFormParentNode(nextTree, nodeId);
    const sectionColumns = clampRaiseSupplyRequestFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampRaiseSupplyRequestFormColSpan(
  ...
  const isLeaf = isRaiseSupplyRequestFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getRaiseSupplyRequestFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampRaiseSupplyRequestFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampRaiseSupplyRequestFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampRaiseSupplyRequestFormColSpan(
...
export function toggleRaiseSupplyRequestFormActive(nodes = [], nodeId) {
  const nextTree = cloneRaiseSupplyRequestFormTree(nodes);
  const existing = findRaiseSupplyRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleRaiseSupplyRequestFormHidden(nodes = [], nodeId) {
  const nextTree = cloneRaiseSupplyRequestFormTree(nodes);
  const existing = findRaiseSupplyRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteRaiseSupplyRequestFormNode(nodes = [], nodeId) {
  const existing = findRaiseSupplyRequestFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneRaiseSupplyRequestFormTree(nodes));
}

export function reorderRaiseSupplyRequestFormNode(nodes = [], nodeId, direction) {
  const existing = findRaiseSupplyRequestFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneRaiseSupplyRequestFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderRaiseSupplyRequestFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findRaiseSupplyRequestFormNodeById(nodes, sourceId);
...
export function getRaiseSupplyRequestFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Raise Supply Request. */
export function leafToRaiseSupplyRequestField(leaf, sectionId) {
