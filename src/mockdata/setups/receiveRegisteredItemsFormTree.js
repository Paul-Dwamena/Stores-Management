/**
 * Receive Registered Items Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  RECEIVE_REGISTERED_ITEMS_FORM_GROUPS,
  RECEIVE_REGISTERED_ITEMS_FORM_SECTIONS,
...
export const RECEIVE_REGISTERED_ITEMS_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const RECEIVE_REGISTERED_ITEMS_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampReceiveRegisteredItemsFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampReceiveRegisteredItemsFormColSpan(value, columns = 2) {
  const max = clampReceiveRegisteredItemsFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampReceiveRegisteredItemsFormColumns(columns);
  // Receive Registered Items defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampReceiveRegisteredItemsFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const RECEIVE_REGISTERED_ITEMS_FORM_LEVEL_1 = RECEIVE_REGISTERED_ITEMS_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampReceiveRegisteredItemsFormColSpan(
 ...
 */
export function buildInitialReceiveRegisteredItemsFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return RECEIVE_REGISTERED_ITEMS_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampReceiveRegisteredItemsFormColumns(section.columns ?? 2);
    const groups = RECEIVE_REGISTERED_ITEMS_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = RECEIVE_REGISTERED_ITEMS_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampReceiveRegisteredItemsFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = RECEIVE_REGISTERED_ITEMS_FORM_FIELD_CATALOG.filter(
...
export function cloneReceiveRegisteredItemsFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneReceiveRegisteredItemsFormTree(node.children || []),
  }));
}

export function isReceiveRegisteredItemsFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isReceiveRegisteredItemsFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenReceiveRegisteredItemsFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenReceiveRegisteredItemsFormTree(node.children, acc);
  });
  return acc;
}

export function findReceiveRegisteredItemsFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findReceiveRegisteredItemsFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findReceiveRegisteredItemsFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findReceiveRegisteredItemsFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getReceiveRegisteredItemsFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findReceiveRegisteredItemsFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findReceiveRegisteredItemsFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getReceiveRegisteredItemsFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findReceiveRegisteredItemsFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findReceiveRegisteredItemsFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getReceiveRegisteredItemsFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getReceiveRegisteredItemsFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveReceiveRegisteredItemsFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isReceiveRegisteredItemsFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveReceiveRegisteredItemsFormLeaves(child));
}

export function collectAllActiveReceiveRegisteredItemsFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveReceiveRegisteredItemsFormLeaves(node));
}
...
export function filterReceiveRegisteredItemsFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getReceiveRegisteredItemsFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenReceiveRegisteredItemsFormTree(nodes)
    .filter((node) => {
      if (isReceiveRegisteredItemsFormValueField(node)) return false;
...
export function addReceiveRegisteredItemsFormNode(nodes = [], payload) {
  const nextTree = cloneReceiveRegisteredItemsFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findReceiveRegisteredItemsFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isReceiveRegisteredItemsFormValueField(parent)) {
  ...
  if (findReceiveRegisteredItemsFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getReceiveRegisteredItemsFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampReceiveRegisteredItemsFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampReceiveRegisteredItemsFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampReceiveRegisteredItemsFormColSpan(
...
export function updateReceiveRegisteredItemsFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneReceiveRegisteredItemsFormTree(nodes);
  const existing = findReceiveRegisteredItemsFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampReceiveRegisteredItemsFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampReceiveRegisteredItemsFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findReceiveRegisteredItemsFormParentNode(nextTree, nodeId);
    const sectionColumns = clampReceiveRegisteredItemsFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampReceiveRegisteredItemsFormColSpan(
  ...
  const isLeaf = isReceiveRegisteredItemsFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getReceiveRegisteredItemsFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampReceiveRegisteredItemsFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampReceiveRegisteredItemsFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampReceiveRegisteredItemsFormColSpan(
...
export function toggleReceiveRegisteredItemsFormActive(nodes = [], nodeId) {
  const nextTree = cloneReceiveRegisteredItemsFormTree(nodes);
  const existing = findReceiveRegisteredItemsFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleReceiveRegisteredItemsFormHidden(nodes = [], nodeId) {
  const nextTree = cloneReceiveRegisteredItemsFormTree(nodes);
  const existing = findReceiveRegisteredItemsFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteReceiveRegisteredItemsFormNode(nodes = [], nodeId) {
  const existing = findReceiveRegisteredItemsFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneReceiveRegisteredItemsFormTree(nodes));
}

export function reorderReceiveRegisteredItemsFormNode(nodes = [], nodeId, direction) {
  const existing = findReceiveRegisteredItemsFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneReceiveRegisteredItemsFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderReceiveRegisteredItemsFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findReceiveRegisteredItemsFormNodeById(nodes, sourceId);
...
export function getReceiveRegisteredItemsFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Receive Registered Items. */
export function leafToReceiveRegisteredItemsField(leaf, sectionId) {
