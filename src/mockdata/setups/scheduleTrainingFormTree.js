/**
 * Schedule training form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  SCHEDULE_TRAINING_FORM_FIELD_CATALOG,
  SCHEDULE_TRAINING_FORM_GROUPS,
  SCHEDULE_TRAINING_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./scheduleTrainingFormFields";
import {
...
export const SCHEDULE_TRAINING_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const SCHEDULE_TRAINING_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampScheduleTrainingFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampScheduleTrainingFormColSpan(value, columns = 2) {
  const max = clampScheduleTrainingFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampScheduleTrainingFormColumns(columns);
  // Schedule Training defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampScheduleTrainingFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const SCHEDULE_TRAINING_FORM_LEVEL_1 = SCHEDULE_TRAINING_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampScheduleTrainingFormColSpan(
...
/**
 * Seed hierarchy: Main holds driver, training name, scheduled date, location, and instructor fields.
 */
export function buildInitialScheduleTrainingFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return SCHEDULE_TRAINING_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampScheduleTrainingFormColumns(section.columns ?? 2);
    const groups = SCHEDULE_TRAINING_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = SCHEDULE_TRAINING_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampScheduleTrainingFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = SCHEDULE_TRAINING_FORM_FIELD_CATALOG.filter(
...
export function cloneScheduleTrainingFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneScheduleTrainingFormTree(node.children || []),
  }));
}

export function isScheduleTrainingFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isScheduleTrainingFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenScheduleTrainingFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenScheduleTrainingFormTree(node.children, acc);
  });
  return acc;
}

export function findScheduleTrainingFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findScheduleTrainingFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findScheduleTrainingFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findScheduleTrainingFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getScheduleTrainingFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findScheduleTrainingFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findScheduleTrainingFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getScheduleTrainingFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findScheduleTrainingFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findScheduleTrainingFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getScheduleTrainingFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getScheduleTrainingFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveScheduleTrainingFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isScheduleTrainingFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveScheduleTrainingFormLeaves(child));
}

export function collectAllActiveScheduleTrainingFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveScheduleTrainingFormLeaves(node));
}
...
export function filterScheduleTrainingFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getScheduleTrainingFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenScheduleTrainingFormTree(nodes)
    .filter((node) => {
      if (isScheduleTrainingFormValueField(node)) return false;
...
export function addScheduleTrainingFormNode(nodes = [], payload) {
  const nextTree = cloneScheduleTrainingFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findScheduleTrainingFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isScheduleTrainingFormValueField(parent)) {
  ...
  if (findScheduleTrainingFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getScheduleTrainingFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampScheduleTrainingFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampScheduleTrainingFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampScheduleTrainingFormColSpan(
...
export function updateScheduleTrainingFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneScheduleTrainingFormTree(nodes);
  const existing = findScheduleTrainingFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampScheduleTrainingFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampScheduleTrainingFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findScheduleTrainingFormParentNode(nextTree, nodeId);
    const sectionColumns = clampScheduleTrainingFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampScheduleTrainingFormColSpan(
  ...
  const isLeaf = isScheduleTrainingFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getScheduleTrainingFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampScheduleTrainingFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampScheduleTrainingFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampScheduleTrainingFormColSpan(
...
export function toggleScheduleTrainingFormActive(nodes = [], nodeId) {
  const nextTree = cloneScheduleTrainingFormTree(nodes);
  const existing = findScheduleTrainingFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleScheduleTrainingFormHidden(nodes = [], nodeId) {
  const nextTree = cloneScheduleTrainingFormTree(nodes);
  const existing = findScheduleTrainingFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteScheduleTrainingFormNode(nodes = [], nodeId) {
  const existing = findScheduleTrainingFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneScheduleTrainingFormTree(nodes));
}

export function reorderScheduleTrainingFormNode(nodes = [], nodeId, direction) {
  const existing = findScheduleTrainingFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneScheduleTrainingFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderScheduleTrainingFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findScheduleTrainingFormNodeById(nodes, sourceId);
...
export function getScheduleTrainingFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Schedule Training. */
export function leafToScheduleTrainingField(leaf, sectionId) {
