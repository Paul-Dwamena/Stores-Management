/**
 * Vehicle document form hierarchy — one fixed Level 1 (Main).
 * Insurance Policy Details and Limits of Liability are locked subfolders.
 * Other fields live directly under Main. L1 is not reorderable.
 */

import { moveTreeSiblingRelative, reorderTreeSibling } from "../../utils/treeReorder";
import {
  DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION,
  VEHICLE_DOCUMENT_FORM_FIELD_CATALOG,
  VEHICLE_DOCUMENT_FORM_GROUPS,
  VEHICLE_DOCUMENT_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./vehicleDocumentFormFields";
import {
...
export const VEHICLE_DOCUMENT_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const VEHICLE_DOCUMENT_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampVehicleDocumentFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampVehicleDocumentFormColSpan(value, columns = 2) {
  const max = clampVehicleDocumentFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampVehicleDocumentFormColumns(columns);
  // Upload Vehicle Document defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampVehicleDocumentFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const VEHICLE_DOCUMENT_FORM_LEVEL_1 = VEHICLE_DOCUMENT_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampVehicleDocumentFormColSpan(
...
/**
 * Seed hierarchy: Main holds document fields plus locked Insurance Policy Details
 * and Limits of Liability folders.
 */
export function buildInitialVehicleDocumentFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return VEHICLE_DOCUMENT_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampVehicleDocumentFormColumns(section.columns ?? 2);
    const groups = VEHICLE_DOCUMENT_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = VEHICLE_DOCUMENT_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampVehicleDocumentFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = VEHICLE_DOCUMENT_FORM_FIELD_CATALOG.filter(
...
export function cloneVehicleDocumentFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...
    children: cloneVehicleDocumentFormTree(node.children || []),
  }));
}

export function isVehicleDocumentFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isVehicleDocumentFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenVehicleDocumentFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenVehicleDocumentFormTree(node.children, acc);
  });
  return acc;
}

export function findVehicleDocumentFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findVehicleDocumentFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findVehicleDocumentFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findVehicleDocumentFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getVehicleDocumentFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findVehicleDocumentFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findVehicleDocumentFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getVehicleDocumentFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findVehicleDocumentFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findVehicleDocumentFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getVehicleDocumentFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getVehicleDocumentFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveVehicleDocumentFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isVehicleDocumentFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveVehicleDocumentFormLeaves(child));
}

export function collectAllActiveVehicleDocumentFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveVehicleDocumentFormLeaves(node));
}
...
export function filterVehicleDocumentFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getVehicleDocumentFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenVehicleDocumentFormTree(nodes)
    .filter((node) => {
      if (isVehicleDocumentFormValueField(node)) return false;
...
export function addVehicleDocumentFormNode(nodes = [], payload) {
  const nextTree = cloneVehicleDocumentFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findVehicleDocumentFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isVehicleDocumentFormValueField(parent)) {
  ...
  if (findVehicleDocumentFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getVehicleDocumentFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampVehicleDocumentFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampVehicleDocumentFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampVehicleDocumentFormColSpan(
...
export function updateVehicleDocumentFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneVehicleDocumentFormTree(nodes);
  const existing = findVehicleDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampVehicleDocumentFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampVehicleDocumentFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findVehicleDocumentFormParentNode(nextTree, nodeId);
    const sectionColumns = clampVehicleDocumentFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampVehicleDocumentFormColSpan(
  ...
  const isLeaf = isVehicleDocumentFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getVehicleDocumentFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampVehicleDocumentFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampVehicleDocumentFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampVehicleDocumentFormColSpan(
...
export function toggleVehicleDocumentFormActive(nodes = [], nodeId) {
  const nextTree = cloneVehicleDocumentFormTree(nodes);
  const existing = findVehicleDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleVehicleDocumentFormHidden(nodes = [], nodeId) {
  const nextTree = cloneVehicleDocumentFormTree(nodes);
  const existing = findVehicleDocumentFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteVehicleDocumentFormNode(nodes = [], nodeId) {
  const existing = findVehicleDocumentFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneVehicleDocumentFormTree(nodes));
}

export function reorderVehicleDocumentFormNode(nodes = [], nodeId, direction) {
  const existing = findVehicleDocumentFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneVehicleDocumentFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderVehicleDocumentFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findVehicleDocumentFormNodeById(nodes, sourceId);
...
export function getVehicleDocumentFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Upload Vehicle Document. */
export function leafToVehicleDocumentField(leaf, sectionId) {
