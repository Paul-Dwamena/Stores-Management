/**
 * Vehicle Request Form hierarchy — one fixed Level 1 (Main).
...
import {
  ...
  VEHICLE_REQUEST_FORM_GROUPS,
  VEHICLE_REQUEST_FORM_SECTIONS,
  withLockedDefaultFields,
} from "./vehicleRequestFormFields";
import {
...
export const VEHICLE_REQUEST_FORM_FIELD_TYPE_OPTIONS = VEHICLE_SPEC_FIELD_TYPE_OPTIONS;

/** Allowed grid column counts for a Level 1 section. */
export const VEHICLE_REQUEST_FORM_COLUMN_OPTIONS = [1, 2, 3, 4];

export function clampVehicleRequestFormColumns(value) {
  const n = Number(value);
  if (!Number.isFinite(n)) return 2;
  return Math.min(4, Math.max(1, Math.floor(n)));
}

export function clampVehicleRequestFormColSpan(value, columns = 2) {
  const max = clampVehicleRequestFormColumns(columns);
  const n = Number(value);
...
function defaultColSpanForFieldType(fieldType, columns = 2) {
  const max = clampVehicleRequestFormColumns(columns);
  // Vehicle Request defaults new custom fields to full row width within the group columns.
  return max;
}
...
export function clampLeavesColSpan(children = [], columns) {
  return children.map((child) => {
    if (child?.key) {
      return {
        ...child,
        colSpan: clampVehicleRequestFormColSpan(child.colSpan ?? columns, columns),
      };
    }
    return {
      ...child,
...
export const VEHICLE_REQUEST_FORM_LEVEL_1 = VEHICLE_REQUEST_FORM_SECTIONS.map((section) => ({
  id: section.id,
  name: section.label,
...
function buildLeafNode(field, parentId, level, sectionColumns, sortOrder, visibleIds) {
  ...
  return {
    ...
    columns: null,
    colSpan: clampVehicleRequestFormColSpan(
...
/**
 * Seed hierarchy: Main holds driver, vehicle, request type, dates, and reason.
 */
export function buildInitialVehicleRequestFormTree() {
  const visibleBySection = withLockedDefaultFields(DEFAULT_VISIBLE_FIELD_IDS_BY_SECTION);

  return VEHICLE_REQUEST_FORM_SECTIONS.map((section, index) => {
    const visibleIds = new Set(visibleBySection[section.id] ?? []);
    const sectionColumns = clampVehicleRequestFormColumns(section.columns ?? 2);
    const groups = VEHICLE_REQUEST_FORM_GROUPS.filter(
      (group) => group.sectionId === section.id,
    );

    const ungroupedFields = VEHICLE_REQUEST_FORM_FIELD_CATALOG.filter(
    ...
    const groupNodes = groups.map((group, groupIndex) => {
      const groupColumns = clampVehicleRequestFormColumns(group.columns ?? sectionColumns);
      const groupNodeId = `${section.id}/${group.id}`;
      const fields = VEHICLE_REQUEST_FORM_FIELD_CATALOG.filter(
...
export function cloneVehicleRequestFormTree(nodes = []) {
  return nodes.map((node) => ({
    ...node,
    options: node.options
      ? node.options.map((opt) => (typeof opt === "string" ? opt : { ...opt }))
      : null,
    children: cloneVehicleRequestFormTree(node.children || []),
  }));
}

export function isVehicleRequestFormLeaf(node) {
  return !node?.children?.length;
}

/** True when the node is a value field (cannot have children). */
export function isVehicleRequestFormValueField(node) {
  return Boolean(node?.key);
}

export function flattenVehicleRequestFormTree(nodes = [], acc = []) {
  nodes.forEach((node) => {
    acc.push(node);
    if (node.children?.length) flattenVehicleRequestFormTree(node.children, acc);
  });
  return acc;
}

export function findVehicleRequestFormNodeById(nodes = [], id) {
  for (const node of nodes) {
    if (node.id === id) return node;
    const match = findVehicleRequestFormNodeById(node.children || [], id);
    if (match) return match;
  }
  return null;
}

export function findVehicleRequestFormParentNode(nodes = [], id, parent = null) {
  for (const node of nodes) {
    if (node.id === id) return parent;
    const match = findVehicleRequestFormParentNode(node.children || [], id, node);
    if (match) return match;
  }
  return null;
}

export function getVehicleRequestFormLevel1Ancestor(nodes = [], nodeId) {
  let current = findVehicleRequestFormNodeById(nodes, nodeId);
  if (!current) return null;
  while (current?.parentId) {
    const parent = findVehicleRequestFormNodeById(nodes, current.parentId);
    if (!parent) break;
    current = parent;
  }
...
export function getVehicleRequestFormNodePath(nodes = [], nodeId) {
  const path = [];
  let current = findVehicleRequestFormNodeById(nodes, nodeId);
  while (current) {
    path.unshift(current.name);
    current = current.parentId
      ? findVehicleRequestFormNodeById(nodes, current.parentId)
      : null;
  }
  return path;
}

export function getVehicleRequestFormDescendantCount(node) {
  if (!node?.children?.length) return 0;
  return node.children.reduce(
    (total, child) => total + 1 + getVehicleRequestFormDescendantCount(child),
    0,
  );
}

/** Active leaf value fields under a subtree (respects inactive ancestors). */
export function collectActiveVehicleRequestFormLeaves(node) {
  if (!node || node.isActive === false || node.isHidden === true) return [];
  if (isVehicleRequestFormValueField(node)) {
    return [node];
  }
  ...
  return children.flatMap((child) => collectActiveVehicleRequestFormLeaves(child));
}

export function collectAllActiveVehicleRequestFormLeaves(nodes = []) {
  return nodes.flatMap((node) => collectActiveVehicleRequestFormLeaves(node));
}
...
export function filterVehicleRequestFormTree(nodes = [], searchQuery = "") {
  const query = searchQuery.trim().toLowerCase();
  if (!query) return nodes;
  return filterNodes(nodes, query);
}

export function getVehicleRequestFormParentOptions(nodes = [], { excludeId = null } = {}) {
  return flattenVehicleRequestFormTree(nodes)
    .filter((node) => {
      if (isVehicleRequestFormValueField(node)) return false;
...
export function addVehicleRequestFormNode(nodes = [], payload) {
  const nextTree = cloneVehicleRequestFormTree(nodes);
  const parentId = payload.parentId || null;
  if (!parentId) {
    throw new Error("Level 1 sections are fixed. Add items under an existing section.");
  }

  const parent = findVehicleRequestFormNodeById(nextTree, parentId);
  if (!parent) throw new Error("Parent node not found.");
  if (isVehicleRequestFormValueField(parent)) {
  ...
  if (findVehicleRequestFormNodeById(nextTree, id)) {
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getVehicleRequestFormLevel1Ancestor(nextTree, parentId) || parent;
  const sectionColumns = clampVehicleRequestFormColumns(l1?.columns ?? 2);
  ...
  const child = {
    ...
    placeholder,
    defaultValue,
    columns: isValueField ? null : clampVehicleRequestFormColumns(payload.columns ?? 2),
    colSpan: isValueField
      ? clampVehicleRequestFormColSpan(
...
export function updateVehicleRequestFormNode(nodes = [], nodeId, payload) {
  const nextTree = cloneVehicleRequestFormTree(nodes);
  const existing = findVehicleRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;

  // Level 1: layout columns only (nested group columns stay independent).
  if (existing.isLocked) {
    const columns = clampVehicleRequestFormColumns(payload.columns ?? existing.columns ?? 1);
  ...
  if (existing.isDefaultLocked) {
    if (!existing.key) {
      const columns = clampVehicleRequestFormColumns(payload.columns ?? existing.columns ?? 2);
      return updateNodeInTree(nextTree, nodeId, {
        columns,
        children: clampLeavesColSpan(existing.children || [], columns),
      });
    }
    const parent = findVehicleRequestFormParentNode(nextTree, nodeId);
    const sectionColumns = clampVehicleRequestFormColumns(parent?.columns ?? 2);
    return updateNodeInTree(nextTree, nodeId, {
      colSpan: clampVehicleRequestFormColSpan(
  ...
  const isLeaf = isVehicleRequestFormLeaf(existing);
  ...
  const maxLength =
    isValueField && fieldTypeSupportsLengthLimits(fieldType)
      ...
      : null;
  const l1 = getVehicleRequestFormLevel1Ancestor(nextTree, nodeId);
  const sectionColumns = clampVehicleRequestFormColumns(l1?.columns ?? 2);
  ...
  return updateNodeInTree(nextTree, nodeId, {
    ...
    columns: isValueField
      ? null
      : clampVehicleRequestFormColumns(payload.columns ?? existing.columns ?? 2),
    colSpan: isValueField
      ? clampVehicleRequestFormColSpan(
...
export function toggleVehicleRequestFormActive(nodes = [], nodeId) {
  const nextTree = cloneVehicleRequestFormTree(nodes);
  const existing = findVehicleRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
  if (existing.isLocked) {
...
export function toggleVehicleRequestFormHidden(nodes = [], nodeId) {
  const nextTree = cloneVehicleRequestFormTree(nodes);
  const existing = findVehicleRequestFormNodeById(nextTree, nodeId);
  if (!existing) return nextTree;
...
export function deleteVehicleRequestFormNode(nodes = [], nodeId) {
  const existing = findVehicleRequestFormNodeById(nodes, nodeId);
  if (existing?.isLocked) {
  ...
  return remove(cloneVehicleRequestFormTree(nodes));
}

export function reorderVehicleRequestFormNode(nodes = [], nodeId, direction) {
  const existing = findVehicleRequestFormNodeById(nodes, nodeId);
  ...
  const { nodes: next, moved } = reorderTreeSibling(
    cloneVehicleRequestFormTree(nodes),
    nodeId,
    direction,
  );
  if (!moved) {
...
export function reorderVehicleRequestFormNodeRelative(
  nodes = [],
  sourceId,
  targetId,
  place = "before",
) {
  const source = findVehicleRequestFormNodeById(nodes, sourceId);
...
export function getVehicleRequestFormLevelLabel(level) {
  return `Level ${level}`;
}

/** Map a leaf node to the field shape used by Vehicle Request. */
export function leafToVehicleRequestField(leaf, sectionId) {
